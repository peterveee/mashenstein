...
const outPath = resolve(process.argv[2] || join(root, 'work', 'auditions', 'misc', 'plumber-audio-audition.wav'));
const musicPath = join(root, 'work', 'tracks', 'plumber-panic.wav');
const effectsDir = join(root, 'work', 'weapons');
const GAP_S = 0.35;
...
for (const name of effects) {
  ...
  cursor += clip.length;
  index.push({ start, end: cursor, label: name, source: join('work', 'weapons', name) });

...
//
// Lifted out of mixer-entry.js whole. Its own header already said it existed to be the
// one place these numbers live; it had simply never left the file. What it does is
// measure the window, plan a height for each region, apply that plan, fit the strips
// inside it, and schedule the whole pass off a frame — plus the splitter, arrangement,
// device and effects drag gestures, which are the same arithmetic driven by a pointer.
//
// It is a layout engine, so it reads a lot of the desk and calls back into it. Both
// directions go through the seam below rather than through the file's scope: what it
// needs to KNOW arrives as values and thunks, what it needs to DO arrives as functions,
// and the desk gets back the dozen entry points it actually calls.
//
...
/**
 * Build the layout engine and wire its drag handles.
 *
 * Everything under "knows" is read, never written; everything under "does" is the desk's
 * own function, so a region that folds because the budget said so folds exactly the way
 * it would have if you had clicked it.
 *
 * @param notesOpen        whether the lower editor is showing (a predicate already).
 * @param selectedLane     which lane is selected, as a thunk — the desk reassigns it.
 * @param isPartHidden     whether a strip part is shed, asked one id at a time.
 * @param rearrangePanelOpen  whether Rearrange is covering the rack, as a thunk. While
 *                         it is, a resize is DEFERRED rather than run against a rack
 *                         nobody can see; `takeDeskFitDebt` is how the desk pays it.
 * @param autoDevH         the device panel's own automatic height, which it writes.
 * @param deferRollResize  the one thing this ever tells the piano roll, so the roll
 *                         itself never has to be handed over.
 */
export function createDeskLayout({
  // knows
  notesOpen, selectedLane, isPartHidden, rearrangePanelOpen, autoDevH, STRIP_PARTS, clamp,
  // does
  toast, deferRollResize, setNotesFolded, setDevicesFolded, setMixerFolded,
  ...
  return {
    // What the desk measures with. `h` is one genuine call site outside this file; `px`
    // has none, and stays because the two are a pair.
    h,
    // The pass itself, and the three ways in to part of it.
    ...
    keepLaneAfterFit: () => { keepLanePending = true; },
    // The upper/lower split, read by the layout preferences and written by its slider.
    workRatio: () => upperWorkRatio,
    writeWorkRatio,
    /** The effects panel's remembered height, or null while it is fitting itself. */
    fxHeight: () => userFxH,
    /**
     * Was a fit deferred while Rearrange covered the rack? Answering CLEARS the debt,
     * so the caller cannot forget to — it was a bare flag on both sides before.
     */
    takeDeskFitDebt: () => {

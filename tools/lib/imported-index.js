...
export function readImported(root) {
  ...
  for (const file of readdirSync(dir).sort()) {
    ...
    // The title is the one the import wrote into its header; the const name is the
    // fallback, which is what it was made from anyway.
    const title = (/^\/\/ (.+?) — imported from/m.exec(src) || [])[1] || m[1];
    out.push({ id: file.replace(/\.js$/, ''), constName: m[1], title, file });
  }
  return out;
}
...
export function writeImportedIndex(root) {
  ...
  for (const e of entries) {
    ...
    for (let i = 2; used.has(name); i++) name = `${e.constName}_${i}`;
    used.add(name);
    local.set(e.id, name);
  }

  const imports = entries.map((e) => (local.get(e.id) === e.constName
    ? `import { ${e.constName} } from './${e.file}';`
    : `import { ${e.constName} as ${local.get(e.id)} } from './${e.file}';`)).join('\n');
  const body = entries.map((e) => `  ${JSON.stringify(e.id)}: `
    + `{ bank: ${local.get(e.id)}, title: ${JSON.stringify(e.title)} },`).join('\n');

...
export function emitEntry(id, preset, { derived = ['id', 'kind', 'level', 'peak', 'factory', 'user'] } = {}) {
  ...
  // One compact routing word before the stack it governs. Osc 1 is always the master;
  // this records which of the other two oscillator phases it resets.
  if (has('sync')) lines.push(`    sync: ${flat(v.sync)},`);
  ...
  // The stage the layers sum into, written directly after them because that is the order

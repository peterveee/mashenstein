...
// Start paying for a tunnel's lower floor before the hero reaches its mouth.
...
// On the upper path, put the surface hero around the middle of the frame. This
// is a presentation anchor only; the simulated surface remains at GROUND_Y.
const TUNNEL_CAMERA_UPPER_PATH_LIFT = 48;
...
// Portrait framing measures the part of a level the player can actually read:
// a standing hero above each floor, plus the 38px cutaway the terrain painter
// exposes below a low road. This keeps Level 1-1's tunnel fossils inside the
// safe frame without making decorative sky or the camera's bottom fill part of
// the gameplay bounds.
const PORTRAIT_TUNNEL_VISUAL_DEPTH = 38;
const PORTRAIT_FRAME_MARGIN = 8;
const PORTRAIT_BOUNDS_SAMPLE_STEP = 4;
...
const PORTRAIT_EDGE_PAN_BOTTOM_MARGIN = 0;
// A lower tunnel can be roughly one authored hero-height below the surface,
// and the portrait zoom magnifies that drop. The old 48px correction was not
...
// A tunnel's portrait pan is a function of the STANDING FLOOR under the hero,
// never of the hero's own altitude: hopping about down there cannot move the
// frame, and a sloped floor moves it exactly as fast as the floor climbs.
//
// It used to be parked instead — re-fitted only when the floor had moved more
// than four pixels since the PREVIOUS FRAME — and a floor climbing at a pixel a
// frame never trips that, so the pan stayed at the chamber's full depth for
// the entire way out (the whole staged exit, five seconds of shelf and climb)
// and then craned the full 336 logical px back to the surface in one move at
// the release. That is the "pans down and hides the background" complaint.
// A high route is a new resting world, not a jump above the old one — but only
// once it is high enough that the standing hero would otherwise be pushed out
// of the top of the gameplay band. Below that the low-ground composition
// simply keeps him higher in the frame, with the lane still where it always
// is, and there is nothing to fix: reframing a sixty-pixel island dragged the
// world down and filled the bottom of the phone with dirt. Peter, on exactly
// that: "we panned DOWN, why? there is plenty of clearance especially in
// portrait mode." This is the floor under that test — the geometric one in
// portraitHighPathFloor is the real gate.
...
//
// It was 16 — the height a cabinet's own hills reach (PROFILES in
// game/terrain.js) — on the argument that a rise the player has already run
...
// two are rolled here.
//
// SURPRISE IS NOT A JUMP FACE. Both 0 and 3 set `surprise`, and 0 is the very
// same face a falling hero wears (updateFallFace), flags and all. Rolled one in
// four each, half of every hop in the game wore it — held for the whole hang
// time, since the roll happens once at launch — so the face that is supposed to
// mean "I did not choose this" was a coin flip on jumps the player had judged
// perfectly. A jump is a thing you decided to do. It gets the two faces of
// somebody who decided to do it.
//
// It costs variety and that is the price: two faces rather than four. The
// alternative was keeping 3 (startled, which at least brings its own raised
// brow), and it still reads as alarm on a hop nobody was alarmed by.
const JUMP_FACES = [1, 2];
// One reroll if the pick repeats the jump before it — the same once-only reroll
// randomPowerPickup uses, so back-to-back hops don't keep landing on the same
// expression. With two faces in the bag that makes them alternate three times
// in four rather than simply flipping a coin each hop.
function rollJumpFace(rng, avoid) {
  const first = JUMP_FACES[rng.int(0, JUMP_FACES.length - 1)];
  return first === avoid ? JUMP_FACES[rng.int(0, JUMP_FACES.length - 1)] : first;
}
...
// THE SHARE PRICE'S INPUTS. The rhythm cabinet's skyline carries a line chart
// on a rooftop, and these are the only numbers that move it. Asymmetric on
...
// update), so the tempo of the run is the tempo of the ride: it stretches out
// over the top, where he is slowest, and gathers again on the way down. The
// rhythm reports the physics and the pitch reports the progress, and between
// them there is nothing left for the sound to lie about.
const LOOP_RUNGS = 12;
// The boost pad's telegraph, in the key of whatever is playing.
//
// `boostTick` is a square at 1500Hz and a sine at 750 scaled by `pitch`, and the
// pad used to sweep that multiplier continuously — 0.85 to 1.35 closing in, then
// 1.35 to 2.9 over the payout. A smooth ramp through a scale hits whatever it
// hits: measured against the SPEED ZONE's E minor, seven of the ten notes a pad
// plays were outside the key, including a G# against the lead's G, which is the
// flat-third clash and the most audible wrong note available.
//
// Snapping costs the ramp its smoothness and that is a real trade — the rise is
// a distance readout as well as a sound. It survives because the approach is
// only three ticks at running speed: three notes are a figure, not a gradient,
// and nobody was reading a curve off them.
const TICK_BASE = 750;          // the cue's own sine, which `pitch` multiplies
const TICK_LO = TICK_BASE * 0.85;
const TICK_HI = TICK_BASE * 3.0;
// The figure to fall back on when there is no song to take one from — OVERTIME
// with the music off, a stage whose bank failed to load, a test harness. Minor
// pentatonic, because every cabinet song in this game is minor.
const LOOP_FALLBACK_SCALE = [0, 3, 5, 7, 10];
// The ceiling the climb's top rung has to stay under, in Hz. Set by the COIN
// cue, which is two square pings at 988 and 1319 — and the ride collects eight
// coins while the climb is running, so the two are not merely both playing, they
// are interleaved. The climb owns the octave below and leaves that band alone.
const LOOP_TOP_HZ = 850;
// The same figure at the cue's own root, for a ride whose notes went missing
// across a snapshot restore — a rewind hands back a loop object, not a lap.
const LOOP_FALLBACK_NOTES = Array.from({ length: 12 }, (_, i) =>
  175 * Math.pow(2, (LOOP_FALLBACK_SCALE[i % 5] + 12 * Math.floor(i / 5)) / 12));

/**
 * The climb's twelve notes, in the key of whatever is playing.
 *
 * A rising run against a song is the one kind of cue that cannot be written down
 * once: fixed semitones are in tune for one track and sour against the next, and
 * this cabinet is worse than most — SPEED ZONE walks its lap through E minor, A
 * minor and B minor, so even one song has three answers. The first cut used a
 * MAJOR pentatonic and the song is built on the minor one, which is the sourest
 * possible near-miss: four notes agree and the third grinds against the lead.
 *
 * So the run is built from the song's own pitch classes at the moment the ride
 * starts (see Audio.songKey), and held for the lap — a figure that modulated
 * halfway round would be following the music rather than playing with it.
 * Rooted an octave or so up from the bass so it sings over the track instead of
 * fighting it, and capped below where the coin cue lives, since eight of those
 * land on top of this while it plays.
 */
/**
 * The nearest note of the song to the ramp position `p`, as a `pitch` multiplier
 * the cue can use unchanged. Falls through to the raw ramp when there is no song
 * to be in key with — OVERTIME with the music off, a bank that failed to load.
 */
function keyedTickPitch(p) {
  const note = Audio.songSnap(p * TICK_BASE, TICK_LO, TICK_HI);
  return note ? note / TICK_BASE : p;
}

function loopClimbNotes() {
  const key = Audio.songKey();
  const classes = key && key.classes.length >= 3 ? key.classes : LOOP_FALLBACK_SCALE;
  // The note is the song's; the OCTAVE is the cue's. The bass sits down at
  // 55-85Hz, far below where a figure like this can sing, so the root is folded
  // up — and how far up is decided by the TOP of the run rather than by the
  // root, because the run is long. Twelve rungs of a five-note scale span 27
  // semitones, so the last note is nearly five times the first: fold by ear to a
  // comfortable root and an A-minor climb tops out at C6, which is exactly where
  // the coin cue lives and undoes the whole reason this sound sits low.
  //
  // So: as high as it will go while the last rung still clears the coins. That
  // is one rule doing both jobs — it puts every key as high as it can sing, and
  // it cannot put any key somewhere that masks the eight coins landing on top.
  const semisOf = (i) => classes[i % classes.length] + 12 * Math.floor(i / classes.length);
  const span = Math.pow(2, semisOf(LOOP_RUNGS - 1) / 12);
  let root = key ? key.root : 175;
  while (root < 55) root *= 2;
  while (root * span > LOOP_TOP_HZ && root > 55) root /= 2;
  while (root * 2 * span <= LOOP_TOP_HZ) root *= 2;
  const out = [];
  for (let i = 0; i < LOOP_RUNGS; i++) out.push(root * Math.pow(2, semisOf(i) / 12));
  return out;
}
...
// How far past the right edge a punted prop rolling out of the frame gets to
// travel before it is retired. It is leaving under its own power and nothing
...
const PROJECTILE_TRAIL_GHOSTS = [
  { distance: 10, alpha: 0.16 },
  { distance: 20, alpha: 0.08 },
];

function projectileTravelDirection(pr) {
  if (pr.type === 'arrow') {
    const slope = ARROW_ARC.a - 2 * ARROW_ARC.b * pr.t + (pr.dive || 0);
    const vx = pr.vx || ARROW_ARC.v;
    const vy = -slope;
    const length = Math.hypot(vx, vy) || 1;
    return { x: vx / length, y: vy / length };
  }
  const x = pr.type === 'axe' || pr.type === 'fist'
    ? (pr.returning || pr.dropping ? -1 : Math.sign(pr.vx || 1))
    : Math.sign(pr.vx || 1);
  return { x: x || 1, y: 0 };
}

function projectileTrailColour(pr) {
  if (pr.type === 'arrow') return '#e3c36f';
  if (pr.type === 'fist') return '#f7bacc';
  if (pr.type === 'axe') {
    if (pr.art === 'bamboo') return '#b7dc8e';
    if (pr.art === 'wrench') return '#b9c0cc';
    return '#ecc3a1';
  }
  const pal = HERO_SPRITES[pr.contactHero]?.pal;
  return pal?.ki || '#f6d33c';
}

function drawGameplayProjectileArt(ctx, pr, x, y) {
  if (pr.type === 'arrow') {
    const slope = ARROW_ARC.a - 2 * ARROW_ARC.b * pr.t + (pr.dive || 0);
    drawRangedProjectile(ctx, 'arrow', x, y + 4, {
      rot: -Math.atan2(slope, pr.rel || ARROW_ARC.v), hero: 'fernwick', flying: true,
    });
  } else if (pr.type === 'axe') {
    ...
    const coral = shotPal?.ki === '#f2767f';
    drawPellet(ctx, x + 3, y + 2, {
      size: pr.size,
      fill: shotPal?.ki,
      hi: coral ? '#ffc0bb' : undefined,
      spark: shotPal?.a,
      orb: !!shotPal?.ki,
    });
  }
...
function drawGameplayProjectileStreak(ctx, pr, x, y) {
  const d = projectileTravelDirection(pr);
  const colour = projectileTrailColour(pr);
  const anchorX = pr.type === 'arrow' ? x : x + 4;
  const anchorY = pr.type === 'fist' ? y + 2 : y + 4;
  const marks = [
  ...
  for (const mark of marks) {
    ...
    ctx.beginPath();
    ctx.moveTo(anchorX - d.x * mark.from, anchorY - d.y * mark.from);
    ctx.lineTo(anchorX - d.x * mark.to, anchorY - d.y * mark.to);
    ctx.stroke();
  }
  if (pr.art === 'wrench') {
    ...
    const px = -d.y, py = d.x;
    ctx.beginPath();
    ctx.moveTo(anchorX - d.x * 12 - px * 1.8, anchorY - d.y * 12 - py * 1.8);
    ctx.lineTo(anchorX - d.x * 12 + px * 1.8, anchorY - d.y * 12 + py * 1.8);
    ctx.stroke();
  }
  ctx.restore();
}

function drawGameplayProjectileTrail(ctx, pr, x, y) {
  if (pr.holdT > 0 || pr.hover || pr.dropping) return;
  const d = projectileTravelDirection(pr);
  const anchorX = pr.type === 'arrow' ? x : x + 4;
  const anchorY = pr.type === 'fist' ? y + 2 : y + 4;

  if (pr.type === 'pellet' && (pr.contactHero === 'kiko' || pr.contactHero === 'clara')) {
    // Small, separated, coreless orb shadows. They are intentionally much
    // smaller than the live shot so Clara's twin bullets never become a third
    // readable round.
    const colour = projectileTrailColour(pr);
    ctx.save();
    ctx.globalAlpha = pr.contactHero === 'kiko' ? 0.2 : 0.18;
    ctx.fillStyle = colour;
    ctx.beginPath();
    ctx.ellipse(anchorX - d.x * 18, anchorY + 0.75, pr.contactHero === 'kiko' ? 2.5 : 1.5,
      pr.contactHero === 'kiko' ? 1.25 : 0.8, 0, 0, Math.PI * 2);
    ctx.fill();
    ctx.restore();
    return;
  }
  ...
  if (pr.type === 'pellet') {
    ctx.save();
    for (const ghost of PROJECTILE_TRAIL_GHOSTS) {
      ctx.globalAlpha = ghost.alpha;
      drawGameplayProjectileArt(ctx, pr, anchorX - d.x * ghost.distance - (pr.type === 'arrow' ? 0 : 4),
        anchorY - d.y * ghost.distance - (pr.type === 'fist' ? 2 : 4));
    }
    ctx.restore();
    return;
  }
  ...
  if (pr.type === 'fist') {
    ctx.save();
    for (const ghost of PROJECTILE_TRAIL_GHOSTS) {
      ctx.globalAlpha = ghost.alpha * 0.8;
      drawGameplayProjectileArt(ctx, pr, anchorX - d.x * ghost.distance - 4,
        anchorY - d.y * ghost.distance - 2);
    }
    ctx.restore();
    return;
  }

  if (pr.type === 'arrow' || pr.type === 'axe') {
    drawGameplayProjectileStreak(ctx, pr, x, y);
    ctx.save();
    for (const ghost of PROJECTILE_TRAIL_GHOSTS) {
      ctx.globalAlpha = ghost.alpha;
      drawGameplayProjectileArt(ctx, pr, x - d.x * ghost.distance, y - d.y * ghost.distance);
    }
    ctx.restore();
  }
}
...
export class RunState {
  ...
  /**
   * Measure the authored vertical envelope once the stage's routes exist.
   *
   * The bounds describe what the portrait player must be able to read, rather
   * than the camera's arbitrary bottom fill: a standing hero above every floor,
   * and the terrain cutaway below it. Sampling the same ground and route
   * functions used by collision/rendering keeps a high fork, a low tunnel and
   * rolling terrain in one coordinate system. The transition points are added
   * explicitly so a short road cannot hide its peak between samples.
   */
  portraitWorldBounds({ includeTunnel = true } = {}) {
    const total = Number(this.totalDist);
    if (!Number.isFinite(total) || total <= 0) {
      return Object.freeze({ top: GROUND_Y - HERO_DRAW_H, bottom: GROUND_Y + PORTRAIT_TUNNEL_VISUAL_DEPTH,
        span: HERO_DRAW_H + PORTRAIT_TUNNEL_VISUAL_DEPTH, topSource: 'base-hero', bottomSource: 'base-cutaway' });
    }
    let top = Infinity;
    let bottom = -Infinity;
    let topSource = 'base-hero';
    let bottomSource = 'base-cutaway';
    const add = (y, source) => {
      if (!Number.isFinite(y)) return;
      if (y < top) { top = y; topSource = source; }
      if (y > bottom) { bottom = y; bottomSource = source; }
    };
    const floorPoints = new Set([0, total]);
    for (const r of this.routes || []) {
      floorPoints.add(Math.max(0, Math.min(total, r.x)));
      floorPoints.add(Math.max(0, Math.min(total, r.x + r.w)));
      for (const f of [r.lip, r.climb, r.hold]) {
        if (Number.isFinite(f)) floorPoints.add(Math.max(0, Math.min(total, r.x + r.w * f)));
      }
    }
    const sample = (x) => {
      const wx = Math.max(0, Math.min(total, x));
      const ground = this.groundYAt(wx);
      add(ground - HERO_DRAW_H, 'base-hero');
      add(ground + PORTRAIT_TUNNEL_VISUAL_DEPTH, 'base-cutaway');
      for (const r of this.routes || []) {
        if (!includeTunnel && r.kind === 'tunnel') continue;
        if (wx < r.x || wx > r.x + r.w) continue;
        const floor = this.routeGroundY(wx, r);
        add(floor - HERO_DRAW_H, `route-${r.kind}-hero`);
        const depth = r.kind === 'tunnel' ? PORTRAIT_TUNNEL_VISUAL_DEPTH
          : r.kind === 'island' ? ISLAND_THICKNESS : 8;
        add(floor + depth, `route-${r.kind}-body`);
      }
    };
    for (let x = 0; x <= total; x += PORTRAIT_BOUNDS_SAMPLE_STEP) sample(x);
    for (const x of floorPoints) sample(x);
    if (!Number.isFinite(top) || !Number.isFinite(bottom)) {
      top = GROUND_Y - HERO_DRAW_H;
      bottom = GROUND_Y + PORTRAIT_TUNNEL_VISUAL_DEPTH;
    }
    return Object.freeze({ top, bottom, span: bottom - top, topSource, bottomSource });
  }

  portraitFrameFit() {
    const portraitConfig = this.portraitConfig();
    if (!this.portraitBounds || !portraitConfig) {
      return Object.freeze({ pan: 0, fits: true, minPan: 0, maxPan: 0 });
    }
    // The portrait frame starts as a fixed composition. The complete level
    // envelope is retained as a diagnostic fit, but it must not pull the
    // camera toward a distant high route: that makes the standing groundline
    // drift down and defeats the phone's larger, ground-up composition. The
    // small edge correction is applied later only when the drawn hero reaches
    // an actual frame extreme.
    ...
    if (this.route?.kind === 'tunnel') {
      ...
      const bounds = {
        top: floor - HERO_DRAW_H,
        bottom: floor + PORTRAIT_TUNNEL_VISUAL_DEPTH,
      };
      const fit = portraitPanForBounds(bounds, portraitConfig.worldZoom,
        GROUND_Y, PORTRAIT_FRAME_MARGIN, 0);
  ...
  portraitHeroPreferredPan() {
    if (!this.portraitGameplay || !isPhonePortraitPresentation() || !this.player) return 0;
    const frame = presentationFrame();
  ...
  portraitHighPathFloor(gameplayEdges = null) {
    ...
    // ONLY IF HE WOULD LEAVE THE BAND. Standing on this route at the resting
    // pan, does the drawn hero's crown clear the top of the gameplay band?
    // Then the frame has room and stays where it is; the edge correction
    // catches anything transient. Only a route that would pin him against
    // the HUD earns the reframe.
    // Judged at the HIGHEST the road gets, not where he is on it: a sprung
    // road is entered at its lip and climbs from there, and a composition
    // that only arrived once the climb had crossed the line would be a pan in
    // the middle of the ride. A road that never leaves the band never earns
    // it; one that will, has it from the launch.
    const bandTop = Number(gameplayEdges?.top);
    if (Number.isFinite(bandTop)) {
      const highest = Number.isFinite(Number(route.rise)) ? Math.max(rise, Number(route.rise)) : rise;
      const crown = screenYFor(base - highest - HERO_DRAW_H, this.camZoom, 0, GROUND_Y);
      if (crown >= bandTop + PORTRAIT_EDGE_PAN_TOP_MARGIN) return null;
    }
  ...
  routeSampleX() {
    ...
    const r = this.route;
    if (!r) return x;
    // `routeRise` is half-open at the far edge (`t >= 1` means the road is
    // already gone), so clamping to `r.x + r.w` would sample the base lane on
    // the very frame the hero walks off the lip. Keep the sample on the last
    // authored column, the same column used by routeExitDrop().
    const last = Math.max(r.x, r.x + r.w - 0.001);
    return Math.min(Math.max(x, r.x), last);
  }
  ...
  routeAt(x0, x1) {
    for (const r of this.routes) {
      ...
      // Once a hero has walked off a raised road, that same road is no longer
      // a candidate for the downward landing sweep. Without this one-way
      // handoff, the footprint can still overlap the lip for a few frames and
      // the sweep reclaims the road after the hero has visibly dropped to the
      // lane, producing the ground flash followed by a snap back up.
      if (r === this.routeReleaseLock) continue;
  ...
  tunnelMouthAt(x0, x1) {
    for (const r of this.routes) {
      ...
      // the far end of the road above. This is the one place that decides
  ...
  updateRoute(prevFeetY, prevToeX) {
    ...
    if (this.route) {
      ...
      // BACK UP ONTO THE ROAD ABOVE.
      //
      // On the way down the slide the lane is only a jump away, and a hero who
      // can still see the road he stepped off ought to be able to get back onto
      // it — the choice at the mouth is a choice, not a trapdoor. Deeper in it
      // simply cannot be reached, because the tunnel is deliberately deeper than
      // anyone in the cast jumps, so this costs nothing at all in the stretch
      // the section is meant to hold you.
      //
      // Swept downward through the lane's own surface, the same test the island
      ...
      const released = this.route;
      this.player.y += drop;
      if (drop > 0.01) this.player.grounded = false;
      this.route = null;
      // A real edge fall stays released until the hero reaches the lower
      // floor. A ramp or zero-height exit is already continuous, so it needs
      // no lock and remains free to resolve normally on the next tick.
      this.routeReleaseLock = !onRoad && drop > 0.01 ? released : null;
      return false;
    }
    ...
    if (hole && this.player.y <= 0.01 && this.player.grounded) {
      ...
      if (fall > 0.01) {
        ...
        // used to be set to 1 to deny that free hop, and on a hero with one
        // jump that denied him the button entirely — grounded false and jumps
        // already at the cap — so from the moment he dropped in until his feet
        // found the slide, pressing jump did nothing at all. Nothing in this
        // game should ever take the jump button away.
        this.player.jumps = 0;
      }
      return false;
    }
    ...
    // Reaching the lower floor ends the one-way edge handoff, but only after
    // this frame has had its chance to reject the just-released route. Clearing
    // the lock before routeAt() let a landing frame reclaim the upper slab when
    // the footprint still touched its lip, producing the same ground flash and
    // snap-back that the lock is meant to prevent.
    if (!is) {
      if (this.player.grounded) this.routeReleaseLock = null;
      return false;
    }
    ...
    if (!crossed && !toeCatch) {
      // routeAt() can see a different road that the grounded hero is not yet
      // crossing. That still counts as reaching the lower floor; do not carry
      // the old edge lock into the next frame and strand the next landing.
      if (this.player.grounded) this.routeReleaseLock = null;
      return false;
    }            // did not cross downward
    this.route = is;
    this.routeReleaseLock = null;
  ...
  /**
   * The lower floor a tunnel is about to put under the frame, or null when
   * there is no tunnel inside the short travel-time look-ahead. Before the
   * mouth this is only a shallow preview: enough to make the lower choice
   * legible without framing it as the route the hero must take. Once the hero
   * clears the mouth, it holds briefly while an upper-path choice settles;
   * after that, the preview is over unless the live route is actually the
   * tunnel. That makes an upper-path jump release the camera deliberately.
   *
   * This is presentation only. `updateRoute` still decides when the hero has
   * actually entered the opening, and the returned floor is combined with the
   * live route floor rather than replacing it.
   */
  tunnelPreviewFloor(heroX) {
    const speed = Math.max(1, Number(this.speed) || Number(this.baseSpeed()) || 1);
    const lookX = heroX + speed * TUNNEL_CAMERA_LOOKAHEAD_SEC;
    const inTunnel = this.route?.kind === 'tunnel';
    const route = inTunnel
      ? this.route
      : (this.routes || []).find((r) => r.kind === 'tunnel'
        && heroX < r.x + speed * TUNNEL_CAMERA_RETURN_DELAY_SEC
        && lookX >= r.x);
    if (!route || (!inTunnel && heroX >= route.x + speed * TUNNEL_CAMERA_RETURN_DELAY_SEC)
      || heroX > route.x + route.w || lookX < route.x) return null;
    const sampleX = Math.min(route.x + route.w, Math.max(route.x, lookX));
    const floor = this.routeGroundY(sampleX, route);
    return inTunnel ? floor : Math.min(floor, GROUND_Y + TUNNEL_CAMERA_PREVIEW_MAX_DROP);
  }
  ...
  updateCamera(dt) {
    ...
    if (portraitActive) {
      ...
      const highPathFloor = !inTunnel ? this.portraitHighPathFloor(gameplayEdges) : null;
      ...
      // IN A TUNNEL THE FRAME IS FITTED TO THE STANDING FLOOR, every frame.
      // On the way in that is the lower route's footprint while the hero is
      // still falling onto it, which is what stops the camera freezing on the
      // surface composition before the fall has reached the controls. From
      // then on it is the ground under him wherever he is along the route —
      // a jump changes his feet, not the floor, so a jump cannot move the
      // frame, and a floor that climbs out brings the frame back with it.
      // The edge fit is a MINIMAL correction from the surface pan, so it
      // returns to zero by itself once the standing hero is back inside the
      // gameplay band, well before the floor has rejoined the lane.
      //
      // The route's AUTHORED depth, not the rolling floor: the lane's hills
      // run on underneath a tunnel, and fitting to the floor itself had the
      // frame breathing forty logical px up and down through the chamber on
      // ground that only rolls. The surface composition ignores hills — the
      // hero sways in a fixed frame — and so does this one.
      const tunnelRise = inTunnel ? this.routeRise(this.routeSampleX(), this.route) : NaN;
      const tunnelDepthFloor = Number.isFinite(tunnelRise) ? GROUND_Y - tunnelRise : floor;
      const feetBounds = { top: feet - HERO_DRAW_H, bottom: feet };
      const panBounds = inTunnel
        ? { top: tunnelDepthFloor - HERO_DRAW_H, bottom: tunnelDepthFloor }
        : feetBounds;
      const standingPan = portraitEdgePanForBounds(panBounds, this.camZoom, GROUND_Y, frameFit.pan,
      PORTRAIT_EDGE_PAN_TOP_MARGIN, PORTRAIT_EDGE_PAN_BOTTOM_MARGIN,
      edgePanLimit, gameplayEdges);
      const edgePan = standingPan;
      ...
      // THE TUNNEL PAN IS ALWAYS EASED, entry included. The entry used to take
      // its whole first step in one frame — the hero can still be falling from
      // the lane and must not spend a frame inside the touch shelf — and that
      // one step was 160 logical px at the end of a roof, taken while he was
      // still standing on the slab, for a floor forty pixels below him. The
      // guarantee it bought is now the feet clamp's below: the drawn hero is
      // corrected against the band every frame, unsmoothed, so the frame
      // moves WITH a fall exactly as far as the fall needs and no earlier.
      // Following also has to stay armed: a transition that had settled would
      // otherwise let the next frame's target land unsmoothed, which on a
      // sloped floor is a pan stepping a pixel at a time with no ease at all.
      const tunnelFollow = inTunnel;
      const shouldStartTransition = (frameChanged || highPathChanged
        || tunnelFollow || tunnelExit)
        && hadPublishedFrame && !modeChanged
        && Math.abs(targetPan - currentPan) > 0.001;
      if (shouldStartTransition) {
        transition = {
          start: transition?.active ? transition.start : currentPan,
          target: targetPan,
          active: true,
          kind: (highPathChanged || previousHighPath || highPath)
            ? 'high-path'
            : (tunnelFollow || tunnelExit ? 'tunnel' : 'frame'),
        };
      ...
      // A genuinely tall underground jump is the one exception to "jumps do
      // not move the frame": the drawn hero is the contract, so if he actually
      // reaches the HUD the eased standing fit is corrected for his real feet
      // — and only then, and NOT eased, because a hero over the HUD is wrong
      // this frame. An ordinary hop stays inside the band and changes nothing.
      if (inTunnel) {
        nextPan = portraitEdgePanForBounds(feetBounds, this.camZoom, GROUND_Y, nextPan,
          PORTRAIT_EDGE_PAN_TOP_MARGIN, PORTRAIT_EDGE_PAN_BOTTOM_MARGIN,
          edgePanLimit, gameplayEdges);
      }
    ...
    // The normal run seeds these values in enter(), but a presentation-mode
    // rotation can be observed by a dev harness before enter has completed.
    // Keep that boundary harmless instead of letting one undefined camera
    // member poison the ordinary framing math with NaN.
    ...
    const heroX = this.playerWorldX();
    // Keep the camera on the route's lip until updateRoute gets its chance to
    // release it. `routeGroundY(heroX, route)` returns the base lane as soon
    // as the hero's anchor is one pixel past the span, but the hero can still
    // have his footprint on the route for that tick and `player.y` is still
    // measured from its floor. Sampling through playerGroundY() clamps to the
    ...
    // preparing to reveal ahead of him.
    const tunnelFloor = this.tunnelPreviewFloor(heroX);
    const speed = Math.max(1, Number(this.speed) || Number(this.baseSpeed()) || 1);
    const tunnelTransition = (this.routes || []).find((r) => r.kind === 'tunnel'
      && heroX >= r.x
      && heroX <= r.x + r.w + speed * TUNNEL_CAMERA_RETURN_DELAY_SEC);
    const upperPathFraming = this.route?.kind !== 'tunnel'
      && tunnelTransition
      && heroX >= tunnelTransition.x + speed * TUNNEL_CAMERA_RETURN_DELAY_SEC
      && heroX <= tunnelTransition.x + tunnelTransition.w;
    const upperPathFloor = upperPathFraming
      ? GROUND_Y + TUNNEL_CAMERA_UPPER_PATH_LIFT : null;
    const releasingTunnelPreview = tunnelFloor == null
      && upperPathFloor == null
      && this.route?.kind !== 'tunnel'
      && this.camFloorY > GROUND_Y + 0.5
      && tunnelTransition;
    ...
    // still up there.
    //
    // The free band is sized for a hero who has JUMPED — below it the frame
    // holds him by craning and nothing about the picture changes, which is
    // exactly right for a hop and for an island. A road is not a hop. Riding one
    // up through the band leaves `lift` at fifty or seventy pixels for the whole
    // climb, and the frame then has to hold the hero's own jump on top of that,
    // which it can only do by pulling the zoom back — so every jump taken on the
    // way up zooms out and every landing zooms back in. Re-pinning from an
    // island's height instead means the anchor has caught up long before the
    // road reaches its entry height, `lift` is back to nothing, and a jump up
    // there costs the same framing as a jump on the ground: a crane, and no
    // scale change at all.
    const band = roadRise < 0 ? CAM_ANCHOR_DOWN
    ...
    const routeAnchorTo = falling
      ? Math.min(GROUND_Y - anchorLift, feetY + fallLead(this.camZoom))
      : GROUND_Y - anchorLift;
    // The preview may carry the anchor a little farther down than the live
    // floor. It is deliberately blended into the existing fall target rather
    // than replacing it, so the actual falling cap still owns a fast unplanned
    // descent. `tunnelPreviewFloor` caps the pre-mouth suggestion; a live
    // tunnel route is allowed to use its full floor.
    const sectionFloor = tunnelFloor == null ? upperPathFloor : tunnelFloor;
  ...
  enter() {
    ...
    this.hitstop = 0;
    this.dead = false;
    // The sink, while one is playing. See updatePitSink.
    this.pitDeath = null;
    // How many times this stage has taken the hero off the bottom of a hole.
    //
    // `||= 0` rather than `= 0`, and that is the whole of it. enter() is not
    // only the start of a stage — it is also the RESTART, the path a death
    // takes when there is no checkpoint behind it yet. Plumber 3's first pit is
    // at 0.12 and the first checkpoint is at 0.33, so every death in that pit
    // came back through here and zeroed the count: the player could drown in
    // the same hole all afternoon and the hint would never arrive, which is
    // precisely the player it exists for.
    //
    ...
    this.route = null;
    // The raised road just left is excluded from landing sweeps until the
    // fall reaches a lower floor. This prevents a lip handoff from becoming a
    // one-frame route reclaim and is carried by rewind/checkpoint snapshots.
    this.routeReleaseLock = null;
    ...
    // but the authored bounds themselves do not move during a run.
    ...
    if (this.portraitGameplay && isPhonePortraitPresentation()) {
      ...
      // easing is reserved for route changes and later viewport changes.
      this.camPan = this.portraitFrameFitState.pan;
      this.prevCamPan = this.camPan;
    }
    ...
    // of being dealt out of the spawner's bag, so every run through the SPEED
    // ZONE has exactly one and it always arrives at the same moment. Something
  ...
  // Portrait pause plates are laid out in the active safe frame rather than
  ...
  portraitPauseSyncButtons(mainButtons = this.portraitPauseButtons()) {
    if (!isPhonePortraitPresentation() || !this.beatLock || !mainButtons?.length) {
      return PAUSE_SYNC_BUTTONS;
    }
    const main = mainButtons[0];
    ...
    return [
      ...
      { id: 'syncReset', x: x + (width + gap) * 2, y, w: width, h, action: 'syncReset', label: 'RESET' },
    ];
  }
  ...
  update(dt) {
    ...
    if (this.finaleT != null) {
      ...
      // The finish dog has the same problem from the other side: it is the one
      // thing still RUNNING when the tape is crossed, and a charging dog that
    ...
    if (this.rewindPlayFrames > 0) {
      ...
      this.rewindPlayFrames--;
      this.rewindCaptureT = 0;
      if (this.rewindPlayFrames > 0 && this.rewindFrames.length > 0) {
        this.rewindFx.tick(dt);
        Input.endFrame();
        return;
      }
      // Finished (counter spent or ring drained): consume the charge NOW,
      // after the LAST restore — every snapshot replayed above still carried
      // active.rewind (writeRewindSnapshot records activePowerups), so a
      // delete any earlier is refunded by the restore and the rewind is
      // infinite. Then fall through to the release edge below for the
      ...
      // of the hold path too — pre-existing). The single-shot story wants the
      // backstop regardless of that quirk.
    ...
    // the ring it is placed so that he lands on the point of the circle the ride
    // has reached. d(camX)/dt comes out at the ride speed times cos(theta) —
    // which IS `sp` at the bottom, so both seams are silent, and which goes
    // negative over the top, which is the U-turn the player is watching.
    // Contact happens up to a body-width before the bottom of the circle — the
    // pad is 18px wide and the hero is 12 — so the ride is armed on contact and
    // held until his own running carries him to the point theta = 0 describes.
    // Starting it early would snap him backward onto the circle on the frame it
    ...
    if (this.loop && !this.loop.pending) {
      ...
      // which is the difference between running the wall and being slid round
      ...
      // still stumbling a second later — a frozen clock is not the same thing as
      // a paused one, and only the position is being driven here.
      for (const k of ['iframes', 'dashT', 'rollT', 'stumbleT', 'slipT', 'landedT', 'boostT']) {
        if (this.player[k] > 0) this.player[k] = Math.max(0, this.player[k] - wdt);
      }
      // The climb. Stepped off the angle he has actually covered, so a rung
      // lands every 30 degrees of ring however long that takes him — the run
      // slows over the top with him and quickens as he comes down.
      const rung = Math.min(LOOP_RUNGS - 1,
        Math.floor((this.loop.theta / (Math.PI * 2)) * LOOP_RUNGS));
      if (rung > (this.loop.rung ?? -1)) {
        this.loop.rung = rung;
        // `pitch` is a multiplier on the cue's own 175Hz root, so the note is
        // handed over as a ratio rather than as a frequency — which keeps the
        // cue's internal balance (the body an octave down, the noise bands)
        // intact whatever key it is transposed into.
        const notes = this.loop.notes || LOOP_FALLBACK_NOTES;
        Audio.sfx('loopRun', {
          pitch: notes[rung] / 175,
          swell: rung / (LOOP_RUNGS - 1),
        });
      }
      // Flagged here, ended after collide(). The last coin on the ring is only a
      // few degrees short of the bottom, so it is collected on the very frame
      // the lap closes — and ending the ride here, before the pickups are swept,
      // threw that coin's tally away and made the full-lap bonus unreachable by
      // construction. The ride has to outlive the frame that finishes it.
    ...
    // Held for the length of a lap. The pad's kick bleeds off over about a
    // second and a half, which is most of the ride — so left to decay the hero
    ...
    if (this.loop && Input.pressed('jump')) {
      // The one button that still means something on the ring. Jumping lets go
      // of it — see bailLoop — which is what keeps a loop a place the player is
      // riding rather than a second of film. During `pending` there is no ride
      // to leave yet, so the same press simply calls the whole thing off and
      // becomes an ordinary jump: you fired the pad and then hopped over the
      // ring, which is a legible thing to have done.
      if (this.loop.pending) {
        this.loop = null;
        if (this.player.jumpPressed(Audio)) this.player.jumpFace = rollJumpFace(this.fxRng, this.player.jumpFace);
      } else {
        this.bailLoop();
      }
    ...
    if (Input.pressed('ability') && !this.loop) {
      ...
      // A card box slot played correctly opens the box, whether or not the
      // round physically gets there — see lightChartBoxOnBeat.
      if (used && this.beatLock && this.checkOnBeat('ability')) this.lightChartBoxOnBeat();
    }
  ...
  heroScreenX() {
    ...
    // ...and the fall into a pit moves it too. Read here with the other two so
    // the splash, the camera and the sprite all agree on where he went in —
    // collisions are over by the time this is non-zero, so nothing else cares.
    if (this.pitDeath) return PLAYER_X + this.pitDeath.dx;
    return PLAYER_X;
  }

  // The portrait character anchor is presentation-only. The simulation keeps
  // PLAYER_X so collision, spawning and runway timing remain shipped; the
  // renderer shifts the complete world by this many world units instead.
  portraitWorldXOffset() {
    if (!this.portraitGameplay || !isPhonePortraitPresentation()) return 0;
    return Number(this.portraitConfig().heroAnchorX) - PLAYER_X;
  }
  ...
  heroScreenRect(z = this.camZoom, pan = this.camPan, floorY = this.camFloorY, standing = false) {
    const heroX = (this.heroScreenX() + this.portraitWorldXOffset()) * z;
  ...
  useAbility() {
    ...
    if (type === 'dash') {
      ...
      // RUSTY'S BAMBOO SHOOT — a disposable third thing to use the axe's
      // outbound motion, after the axe itself and the wrench. Same wind-up:
      // nothing leaves on the press, the cane goes at the throw's release beat
      // (the pull-whip gesture in drawHumanoid, keyed on `spec.bundle`). It
      // breaks on its first contact; Rusty's dispenser supplies the next one.
  ...
  // The slide plow's flourish, on top of the break's own scatter. Two things
  // separate a box you KICKED from a box that merely died, and the old cue —
  // the borrowed weapon crash, a ring of gold sparks centred on the crate — had
  // neither of them:
  //
  //   it is low. The leg goes through the BOTTOM slat, not the middle, so the
  //   flash belongs at boot height. A burst centred on the box read as the box
  //   detonating on its own account.
  //   it is DIRECTIONAL. The hero is travelling at 160+ and the pieces go with
  //   him. An even bloom is what a shot box does standing still; a plowed one
  //   throws its slats downrange, and that difference is the whole reason you
  //   can tell from across the room which of the two just happened.
  //
  // Speed rides `this.speed` so a plow at full pelt throws further than one at
  ...
  plowBurst(ob, cx, cy) {
    ...
    const floor = this.entityGroundY(ob);
    // Contact, at the boot rather than at the centre of mass.
    const bx = cx + 2, by = Math.min(floor - 2, cy + ob.h * 0.25);
    burst(bx, by, 10, 120, 0.2, '#fff8e8', 1.5, 60, r);
    burst(bx, by, 8, 150, 0.3, '#f6d33c', 1.2, 150, r);
    // The slats, thrown forward and up — 5 to 45 degrees off the horizon, so
    // they arc over the road ahead and land in front of him.
    const sp = 90 + this.speed * 0.55;
    for (let i = 0; i < 10; i++) {
      const a = -(0.08 + r() * 0.7);
      const s = sp * (0.55 + r() * 0.7);
      spawnShard(bx, by - r() * 6, Math.cos(a) * s, Math.sin(a) * s,
        0.55 + r() * 0.4, colors[i % colors.length],
        2.4 + r() * 2.6, 2.4 + r() * 2.6, (r() - 0.5) * 30, 420, floor);
    }
    // Dust off the road. The boot is at ground level through all of this, so
    // the surface gets kicked as well as the crate.
    for (let i = 0; i < 6; i++) {
      spawn(cx - 4 + r() * 14, floor - 1 - r() * 3, 40 + r() * 90, -10 - r() * 30,
        0.35 + r() * 0.25, '#d8c8a8', 1.4, 40);
    }
  }
  ...
  jugglePunt(ob) {
    const n = juggle(ob, this.speed, puntTuneFor(ob));
  ...
  /**
   * A round reaches a card box. Nothing breaks; a fuse starts.
   *
   * THIS IS THE WHOLE MECHANIC. The cabinet's contract is that both halves of
   * the figure land on the grid — the shot, which the judge already scores
   * (checkOnBeat('ability')), and the explosion, which nothing could put on a
   * beat by aiming, because the six ranged weapons in the cast close on a box
   * at anywhere from 354px/s to 657 (see BOX_LEAD_BEATS). So the burst is not
   * timed off the flight at all: it is BOX_BURST_BEATS after the beat the chart
   * asked the shot on, which is a number the lane wrote down when it laid the
   * box and every hero arrives comfortably inside.
   *
   * The fallback matters as much as the rule. A shot fired well off the beat —
   * or at a box whose own beat has already gone by — still gets a quantized
   * burst, on the next line the clock has not passed yet. A box that popped the
   * instant a late round touched it would be the one thing on a beat stage
   * happening off the beat, which is worse than being late.
   *
   * Returns false for anything that is not a card box, so the ordinary
   * projectile path is untouched.
   */
  lightCardBox(ob) {
    ...
    if (!Number.isFinite(beat)) {
      ...
      // An unquantized break is honest; a box that never opens is not.
  ...
  /**
   * The ability button, played on a card box's own beat: light the box.
   *
   * THE PRESS IS THE AIM. Nothing in an auto-runner is aimed — the hero's lane,
   * his height and his distance from the box are all the game's to decide — so
   * the only thing the player contributes to a shot is WHEN, and the cabinet
   * has already judged that (checkOnBeat consumed the slot before this is
   * called). Arming here rather than waiting for contact is what makes the box
   * the same figure for every hero the lane deals it to: Grumpos' axe arrives
   * at a box a beat and a bit after it is thrown and Clara's slug arrives in
   * under one, and a mechanic that read the arrival would be giving the four of
   * them four different beats to play.
   *
   * The round still flies, and it still lights a box it reaches on its own —
   * that is the path for a shot fired OFF the beat, which gets a quantized
   * burst like any other (see lightCardBox). This is only the guarantee that a
   * beat played right is a beat that pays.
   */
  lightChartBoxOnBeat() {
  ...
  burstCardBox(ob) {
    ...
    burst(cx, cy, 14, 95, 0.55, '#f6d33c', 2, 120, () => this.fxRng.float());
    // `quiet`: blockBreak is already the cue, and stacking the generic crunch
    // under it only muddies the front edge — the same argument the slide plow
    // makes at its own call.
    this.breakObstacle(ob, false, true);
    this.score += 30;
  }

  // `quiet` keeps everything except the break's OWN cue, for the one caller
  // that has already played a louder one over the top of it — the slide plow
  ...
  breakObstacle(ob, silent, quiet) {
    ...
    if (!silent) {
      const cy = this.entityGroundY(ob) - ob.alt - ob.h / 2;
      if (ob.def.qbox) { if (!quiet) Audio.sfx('blockBreak'); this.qboxPop(cx, cy); }
      else {
        if (!quiet) Audio.sfx('crunch');
        shake(0.8, 0.08);
  ...
  //
  // BOTH RULES ARE ABOUT THIS FLOOR. A portal stands on the LANE, and the sweep
  // used to ask nothing but "does this share my x" — so a barrel rolling along
  // a tunnel road ninety-six pixels UNDER the portal was deleted for being
  // beneath it, in plain sight, while the hero was still coming down the tunnel
  // toward it. Nothing down there is in the approach (a hero on that road
  // cannot reach the portal at all) and nothing down there is drawn on the
  // portal, so nothing down there is the portal's business. And because the
  // sweep re-runs every frame while the portal is alive, a hazard that MOVES
  ...
  updateEntities(dt, sp) {
    ...
    // The unwrapped chart beat, for anything quantized rather than merely
    // pulsed: `beat` above is a 0..1 phase and cannot say WHICH beat it is.
    const chartBeat = this.beatLock ? this.rhythmBeatForJudging() : null;
    // A ROUTE'S HAZARDS DO NOT MOVE UNTIL THEY ARE NEARLY IN SHOT.
    //
    // The lane's spawner lays each hazard just off the right edge, so a barrel
    // there has a screen's width to roll and no more — which is the runway it
    // was tuned against. A ROUTE is laid all at once, the whole two thousand
    // pixels of it, the moment the camera comes within sight of its mouth: every
    // barrel on it then starts rolling toward a hero who is still twelve seconds
    // away, and at 40px/s one of them crosses five hundred pixels before he
    // arrives. They bunch up behind the entrance and come at him out of nothing,
    // which is not a hazard, it is an ambush.
    //
    // The gate is on a ROUTE'S hazards and on SELF-MOTION only. The lane's own
    // flyers and rollers are laid a screen ahead by the spawner and have always
    // been free to drift from the moment they exist; taking that away would
    ...
    for (const ob of this.obstacles) {
      ...
      // The finish dog announces itself and keeps announcing itself, the whole
      // way across the screen. Three rules govern the voice:
      //
      // SYNCED TO THE JAW. The rig's gape rides the stride (animals.js: widest
      ...
      // the ear exactly as it does on the screen, and the last bark is gone
      // before the finish fanfare wants the room.
      //
      ...
      if (ob.type === 'finishDog') {
        ...
        const fps = propFps(ob.skin || ob.type);
      ...
      // both be steering it.
      if (ob.punted) {
        stepPunt(ob, dt, puntTuneFor(ob));
        // A prop rolling out of the frame is the one thing in the world that
        // leaves to the RIGHT, and the lane's cull only ever looks behind — so
      ...
      // The loop pad shares it wholesale. It is the same promise made by the
      // same kind of object — run over this and something good happens — so it
      // gets the same rising line on the way in and the same fall off a cliff
      // when the chance has gone, and a player who has learned one has learned
      // the other.
      if (ob.def.isBoost || ob.def.isLoop) {
        ...
        // The whistle fires ONCE, here, on the frame the chance is gone. It is
        // one continuous gesture rather than a run of notes, so there is nothing
        // for the update loop to keep feeding — and feeding it WOULD break it,
        // since a second overlapping copy is exactly the two-note failure the
        // cue was rebuilt to avoid. It also outlives BOOST_MISS_TAIL: the tail
        // is how long the CHEVRONS take to wind down, and the sound is allowed
        // to finish falling after the pad has stopped moving.
        if (!ob.used && !ob.missed && gap + ob.w < 0) {
          ob.missed = true;
          ob.missT = BOOST_MISS_TAIL;
          Audio.sfx('boostFall');
        }
        if (ob.missed) {
        ...
        if (ob.arm > 0.12 && !ob.used && !ob.missed) {
          ...
          if (ob.tickT <= 0) {
            ...
            Audio.sfx('boostTick', { pitch: keyedTickPitch(0.85 + 0.5 * ob.arm) });
          }
          ...
          if (ob.tickT <= 0) {
            ob.tickT = 0.045;
            Audio.sfx('boostTick', { pitch: keyedTickPitch(1.35 + 1.55 * (1 - ob.hitT / BOOST_HIT_TAIL)) });
          }
        }
        // No miss branch here. It used to replay a tick at falling pitch every
   ...
   * the hero and the thing they threw at is not the thing they threw at. The
  ...
  updateProjectiles(dt, sp) {
    for (const pr of this.projectiles) {
      ...
      if (pr.type === 'axe' || pr.type === 'fist') {
        ...
        pr.t += dt;
        const disposable = pr.art === 'bamboo';
        if (disposable) {
          // Rusty has an endless supply in the dispenser. Bamboo is a
          // one-contact throw: it keeps its outbound flight, then breaks on
        ...
        if (!disposable && pr.hover && !pr.returning) {
          pr.hoverT += dt;
        ...
        if (!disposable && !pr.returning) this.homeRound(pr, dt);
        ...
        if (!disposable && pr.returning && !pr.dropping) {
        ...
        } else if (!disposable && pr.dropping) {
      ...
      if (pr.type === 'pellet' || pr.type === 'arrow' || pr.type === 'axe' || pr.type === 'fist') {
        for (const ob of this.obstacles) {
          ...
          if (overlaps(box, pbox)) {
            ...
            // THE CARD BOX DOES NOT BREAK WHEN IT IS HIT. It lights, and it
            // goes on the beat — see lightCardBox and BOX_BURST_BEATS. The
            // round is spent either way (a returning weapon parks and comes
            // home; bamboo breaks on contact), because a pellet that
            // carried on through would arrive at the NEXT box a beat early.
            if (this.lightCardBox(ob)) {
      ...
      // axe, a bamboo cane and a thrown fist all flew clean through the drawing,
      // and a shot
      ...
      // anything unbreakable: a returning weapon parks and comes home, bamboo
      // breaks, and a pellet dies unless it pierces.
      //
      ...
      if (pr.live && this.copter && !this.copter.flyOff
        ...
        if (!pr.hitIds.has('copter') && overlaps(box, pbox)) {
          ...
          if (pr.art === 'bamboo') pr.live = false;
          else if (pr.type === 'axe' || pr.type === 'fist') { pr.hover = true; pr.hoverX = pr.x; }
          else if (!pr.pierce) pr.live = false;
        }
      }
    ...
    // A held or hovering axe-type weapon belongs to its thrower — another hero
    // cannot catch it. Three heroes fly this type now (Grumpos's axe, Lorenzo's
    // wrench, Rusty's cane), so the check is per-projectile: swapping to Lorenzo
    // used to
  ...
  rhythmRequiredAt(beat, chart) {
    ...
    // hole is there, and the scoreboard can read the chart and be right. A card
    // box is not: the lane skips it for a hero with no weapon and skips it
    // again on the quiet pass of an `every` cadence (see BeatSpawner.fill), and
    // scoring a shot nobody could fire at nothing that was standing there would
    // break a combo the player did everything right to hold.
    //
    // So the judge asks the LANE rather than the chart. `eventInstances` is the
    // spawner's own record of what it actually laid, stamped with the same
    // unwrapped beat number this method is being asked about, and it is cleared
    // and rebuilt by every resync — so it is never a stale answer.
  ...
  /**
   * A hit, as the skyline's share price hears about it.
   *
   * Kept separate from the miss step because it is a different event and worth
   * far more: missing a beat is a bad quarter, being hit is the news. It also
   * pulls the thumb off the board immediately — a celebration still standing
  ...
  /**
   * Off the ring at the bottom, back into the run.
   *
   * The handover is a whole frame's worth of state because the ride has been
   * lying to the physics for a second: `grounded` was false so nothing would
   * catch him, `vy` was pinned so nothing would pull him, and `y` was being
   * written from the circle. All three go back to what standing on the road
   * means, and the jump budget is cleared with them — coming off a loop with a
   * spent double jump because of what the ride left behind would be a punishment
   * for taking it.
   */
  endLoop() {
    const done = this.loop && this.loop.got >= this.loop.total;
    this.loop = null;
    const p = this.player;
    p.y = 0; p.vy = 0;
    ...
    // cannot end in a hit he had no way to see coming.
    p.iframes = Math.max(p.iframes, LOOP.exitIframes);
    // The boost lean and the ground rush, fired at the EXIT rather than the
    // entrance. Both are hero-side treatments built around a hero leaning into
    // the run, and through the lap he is not doing that — he is sideways, and
    // then upside down. Coming out is the moment they read.
    p.boostT = BOOST_LEAN_T;
    this.speedBoost = Math.min(1.0, this.speedBoost + LOOP.exitBoost);
    Audio.sfx('boost');
    burst(this.camX + PLAYER_X + 6, this.playerGroundY() - 6, 10, 90, 0.45, '#f6d33c', 1, 100,
      () => this.fxRng.float());
    if (done) {
      this.score += LOOP.bonusScore;
      this.coins += 5;
      Audio.sfx('perfect');
      this.floatText('FULL LOOP.', '#f6d33c');
    }
  }

  /**
   * Letting go of the ring, mid-lap.
   *
   * The reason the ride is not a cutscene. A loop the player cannot leave is a
   * second of film in the middle of a runner, and the button that would
   * otherwise do nothing for that second is the obvious one to give it back to.
   *
   * What he leaves with is the TANGENT, and only its vertical half: the forward
   * half is the run, which never stopped and would be double-counted if it were
   * handed back here. So bailing on the way up is thrown clear over the ring,
   * and bailing past the top drops him off the underside — which is the honest
   * consequence and costs him nothing but the rest of the coins, because
   * falling has never hurt anybody in this game.
   */
  bailLoop() {
    if (!this.loop || this.loop.pending) return;
    const vy = loopExitVy(this.loop, this.speed);
    const alt = this.player.y;
    this.loop = null;
    const p = this.player;
    p.y = alt;
    p.vy = Math.max(TERMINAL_VY, Math.min(BASE_JUMP_V, vy));
    p.grounded = false;
    // `launched`, exactly as a spring pad leaves it: it exempts the arc from the
    // variable-jump cut (a bail is not a held button) and keeps him drawn as a
    // hero in flight rather than a hero falling. One air jump left, so a bail is
    // a move you can follow up rather than a hole you dropped into.
    p.launched = true;
    p.jumps = 1;
    p.jumpFace = rollJumpFace(this.fxRng, p.jumpFace);
    Audio.sfx('jump');
  }

  /**
   * Stand the run's one loop-de-loop up, once it is nearly on screen.
   *
   * Lazy for the same reason the spring pads are: the lane it has to clear does
   * not exist until the spawner has laid it, so a ring placed at the top of the
   * stage would be sweeping empty ground and would find the cones only later,
   * standing in its own footprint. Placing it at the same 240px lead as a road
   * entry puts it just outside the view, after the patterns around it are down
   * and before the player can see anything move.
   *
   * The ring is not a route and cannot be one — see game/loop.js — so nothing
   * here builds geometry. It plants a pad, hangs the coins on the circle the pad
   * implies, and clears the ground the ride starts and finishes on.
   */
  spawnLoopSetPiece() {
    if (this.loopAt == null || this.loopSpawned) return;
    // Planted INSIDE the stretch the lane has already been filled over, so the
    // first sweep clears real patterns rather than empty ground. The spawner
    // fills to camX + 680 and the ring's footprint runs ~136px past its pad, so
    // 520 keeps the whole thing behind that frontier — while still being twice
    // the width of the view, so nothing is ever seen appearing.
    if (this.camX + 520 < this.loopAt) return;
    // Never inside the finishing straight. It cannot happen at 0.55 of the
    // stage, and it is one line to make that a fact rather than an assumption.
    if (!this.overtime && this.loopAt + LOOP.r + 120 > this.finishWorldX()) { this.loopSpawned = true; return; }
    this.loopSpawned = true;
    const def = OBSTACLES.loopPad;
    const px = this.loopAt - def.w / 2;
    const pad = makeObstacle('loopPad', px, {});
    // The road's own colours, carried on the entity so the ring can bury its own
    // feet in them. draw.js is handed the style pack, not the cabinet, and the
    // alternative — teaching every entity painter about cabinet palettes — is a
    // lot of plumbing for the one prop that has to look like it was BUILT here
    // rather than parked here.
    pad.groundCol = this.cabinet.ground;
    pad.groundDark = this.cabinet.groundDark;
    this.obstacles.push(pad);
    // The coins belong to this ring, so the ride can tell whether it took all of
    // them without counting every coin the hero picked up on the way in.
    for (const c of loopCoinSpots(this.loopAt)) {
      const coin = makePickup('coin', c.x, c.alt);
      coin.loopId = pad.id;
      this.pickups.push(coin);
    }
    this.clearLoopLane();
  ...
  /**
   * Keep the ring's ground clear, and keep it clear.
   *
   * EVERY FRAME, not once when the pad is planted. The lane is filled ahead of
   * the camera on its own clock and its frontier sits BEHIND where the ring
   * lands, so a one-shot sweep runs before most of the patterns around the ring
   * exist — and the crate that turned up afterwards was laid straight across the
   * mouth, standing on the pad, inside the loop. Same trap `clearRouteHazards`
   * documents one system over, and the same answer: this only ever clears
   * `live`, so re-running it costs nothing and can never undo itself.
   *
   * What it protects is two different things at once. The approach is the one
   * stretch where the player is DECIDING rather than dodging — jump here and the
   * ride is forfeit — and a cone in it turns that choice into a scramble. The
   * far side is where the ride puts him back on the ground at speed, having
   * spent a second looking at the inside of a circle, so a hazard waiting there
   * is a hit he had no way to see coming.
   */
  clearLoopLane() {
    // Read out of the live list rather than held in a field. The pad is culled
    // when it goes behind the camera and rebuilt wholesale by both snapshot
    // systems, and a stored reference would survive all three as a pointer to an
    // object nothing else can see any more.
    ...
    // the set piece — the eight that trace the ride stop being legible as the
    // thing the ride is for. Only coins, and only the ones actually inside the
    // ring: a capsule is rare enough to be worth keeping wherever it fell, and
    // the run-up is left alone so the approach still pays.
    const cx = pad.x + pad.w / 2;
    for (const p of this.pickups) {
      if (!p.live || !p.def.coin || p.loopId === pad.id) continue;
      const dx = p.x + p.w / 2 - cx;
      const dy = p.alt + p.h / 2 - LOOP.r;
      if (dx * dx + dy * dy < LOOP.r * LOOP.r) p.live = false;
    }
  }
  ...
  /**
   * Every place a route makes the lane unfair, swept together.
   *
   ...
   *
   * FOUR — the run-up to a raised mouth. A jump taken to clear a hazard that
   * close to the ledge is still in the air when the ledge arrives, so it comes
   * down on the slab: the obstacle, not the player, chooses the high road. See
   * the window at `entryFrom`.
   *
  ...
  clearRouteHazards() {
    ...
    for (const is of this.routes) {
      ...
      // FOUR — the run-up to the mouth. A hazard standing a jump's length in
      // front of a raised road takes the choice away twice over: not jumping is
      // a hit, and the jump that clears it is still in the air when the mouth
      // arrives, so it lands on the slab. The player is put on the high road by
      // an obstacle rather than by deciding to go up there — and the high road
      // is not free, it ends in a drop whose timing they do not choose either.
      // The mouth is a ledge, so the whole approach is the choice; this keeps
      // it one.
      //
      // A jump's length is the honest width: the worst hero's airtime at this
      // stage's speed (~72px in world 1, ~133 on UNPLUGGED), which is exactly
      // the strip where a clearing jump cannot come down short of the mouth.
      // Anything further back lands on the lane with the choice still open.
      //
      // Only where the mouth is actually catchable. A sprung or stacked road
      // sits above every hero's apex (routes.js exempts those from
      // MAX_ISLAND_RISE) — a jump there clears the hazard and comes down on the
      // lane, which is the ordinary case the spawner already budgets for.
      const entryFrom = (is.entry > 0 && is.entry <= worstJumpApex())
        ? is.x - worstAirtime() * this.speed
        : null;
      ...
      // One window for everything swept around a tunnel's openings — the coins
      // below and the hazard sweep after them measure against the same span,
      // because "close enough to the lip to be unfair" is one distance, not two.
      const holeClear = pitClearance(this.spawner.react, this.speed);
      {
        const clear = holeClear;
      ...
      for (const ob of this.obstacles) {
        ...
        if (is.kind === 'tunnel') {
          ...
          // use, because it knows one opening this loop's old hand-built list
          // did not: the ROOF GAP, the open stretch past the road's end where
          // the lane deliberately stops existing so the top has a way out. A
          // hazard laid there stood on the lane line with nothing drawn under
          // it — a cactus in mid-air past the end of the roof.
          // Two windows, honest to two different reasons. ON an opening (±8)
          // the drawing itself is wrong — the thing stands on nothing — so it
          // goes whatever it is. NEAR one, within the same clearance the pits
          // and the coin sweep use, only what demands an ACTION goes: a hazard
          // at the lip is the "crate two pixels past the far lip" unfairness —
          // the jump it asks for and the drop off the edge are one move the
          // player does not get to separate — while a sign or a decoration
          // asks nothing and may stand. Through `retireExit` rather than a
          ...
          if (ob.def.action !== 'none' && ob.x + ob.w >= exitFrom
            && ob.x <= exitFrom + this.spawner.react * this.speed) retireExit(ob);
          continue;
        }
  ...
  spawnScriptedRewindMaybe() {
    ...
    this.drip.notePower(x, 'capRewind');
  }

  // The plumber gatekeeper: one unkillable dog holds the finish line. It
  // spawns just this side of the tape as the pole comes into view and charges
  // the hero, barking (see the finishDog block in updateEntities). Scripted
  // rather than dealt from the pattern bag: the finishing straight is
  // deliberately empty (see FINISH_CLEAR), so the last thing every plumber
  // stage asks for is one clean jump over something nothing in the kit can
  // remove. `breakable: false` on the def is the unkillable part; the ordinary
  ...
  /**
   * Plant the stage's scripted pits.
   *
   * Lazy, and for the same reason the loop-de-loop is: the lane a hole has to
   * be cut into does not exist until the spawner has laid it, so a gap dropped
   * in at the top of the stage would be carved through empty ground and would
   * meet its cones later, standing in mid-air over it. Planting inside the
   * stretch the spawner has already filled means the sweep below clears real
   * patterns.
   *
   * And it SWEEPS. A hole is the one hazard whose fairness is not about
   * reaction time but about run-up: the spawner's own gap patterns get their
   * approach for free because `fairGap` budgeted for them, and these did not
   * pass through the spawner at all. So the pit clears its own approach and its
   * own landing, and what it clears is a jump's worth on either side rather
   * than a token margin — a crate two pixels past the far lip is a death the
   * player could not have avoided, which is exactly what a fatal hazard may
   * never be.
   */
  spawnScriptedPits() {
    if (this.beatLock) {
    ...
    for (const plan of this.pitPlan) {
      ...
      // Far enough ahead that the pit AND the hint in front of it are both off
      // screen. 520 was outside the view for the hole itself and not for its
      // sign, which stands 65px short of the lip — so a scripted pit could
      // never be signed at all, because by the time it existed its sign
      // position was already inside the frame. The sweep below still runs
      // against real patterns: the spawner fills to camX + 680, and nextX is
      // pushed past the landing so nothing is laid into the clearance after.
  ...
  // teaching problem before it is a difficulty one. The honest fix is not to
  // make the hole smaller — it is to make sure the player knows a hole is
  // coming, which is exactly what the bake-off found the hole itself cannot
  // reliably say on its own. So the run says it, in the lane, in a word.
  //
  // TWO, not one. One failure is a mistimed jump, and a hint after it would be
  // the game explaining a control the player already has; two is a pattern, and
  // a pattern is worth answering. The counter is per stage and deliberately
  // survives a checkpoint restore — the player does not forget having drowned
  // in tar just because the run rewound, and a counter that reset with the
  // snapshot could never reach two on a stage with a checkpoint before the
  // first pit.
  //
  ...
  signPits() {
    ...
    for (const ob of this.obstacles) {
      ...
      // Measured from where the sign will stand rather than from the lip, or a
      // hole that qualified by a few pixels would pop its own hint into frame
      // beside the player, which reads as scenery appearing and teaches nothing.
      const x = ob.x - SIGN_LEAD - OBSTACLES.jumpSign.w;
      if (x < this.camX + W + 16) continue;
      ob.signed = true;
      ...
      if (!clear) continue;
      const sign = makeObstacle('jumpSign', x, {});
  ...
  checkCheckpoints() {
    ...
    // Never mid-lap. `distance` is the camera, and on a ring the camera swings
    // forward and back through the same span rather than advancing — so a
    // checkpoint line lying under a loop would be crossed on the way up, and the
    // snapshot taken there would restore the player into the middle of a ride
    // that no longer exists. The set piece is placed between the two checkpoints
    // for this among other reasons; the guard is what makes it a fact.
    if (this.loop) return;
  ...
  makeSnapshot() {
    return {
      ...
      // checkpointed at a third and then died would come back with the flag
      // still set and the pad no longer in the world, and the set piece would
      // simply never happen on the retry.
      loopSpawned: this.loopSpawned,
      // Which of the stage's scripted pits have been planted. Same one-shot
      // problem the loop has, and a worse consequence: without this a player
      // who died in the third pit would come back to a stage that no longer
      // has one, because the plan was already marked done. The snapshot is
      ...
      routeReleaseLock: this.routes.indexOf(this.routeReleaseLock),
  ...
  writeRewindSnapshot(s) {
    ...
    s.routeReleaseLock = this.routes.indexOf(this.routeReleaseLock);
    // The ride, if one is running. Written into a slot the pool owns rather than
    ...
    if (this.loop) {
      ...
      ls.theta = this.loop.theta; ls.pending = this.loop.pending;
      ls.got = this.loop.got; ls.total = this.loop.total; ls.done = !!this.loop.done;
      // Which rung of the climb has sounded. Rewind without it and the run
      // re-fires every note it already played as the tape rolls forward again.
      ls.rung = this.loop.rung ?? -1;
      s.looping = true;
    } else {
      s.looping = false;
    }
    ...
    // copySetInto, not assignInto — see its note: a Set copied by reference
    // would let the recorded past keep changing to match the present.
    s.spawnerUsedOnce = copySetInto(this.spawner.usedOnce, s.spawnerUsedOnce);
  ...
  restoreRewindSnapshot(s) {
    ...
    // reused every frame, so a shared object reference would be the live ride.
    if (s.looping) {
      this.loop = this.loop || {};
      this.loop.obId = s.loop.obId; this.loop.cx = s.loop.cx; this.loop.r = s.loop.r;
      this.loop.theta = s.loop.theta; this.loop.pending = s.loop.pending;
      this.loop.got = s.loop.got; this.loop.total = s.loop.total; this.loop.done = s.loop.done;
      this.loop.rung = s.loop.rung ?? -1;
    } else {
      this.loop = null;
    }
  ...
  collide() {
    const playerX = this.playerWorldX();
    const pbox = this.playerBox();
    // On the ring, nothing in the lane can touch him. He is up the wall or over
    // the top, and the lane's own furniture is at ground level under a piece of
    // track — being hit by it would be the geometry showing through. The
    // PICKUPS below still run, and that is not an exception made for the loop:
    // the ring's coins are ordinary coins at ordinary altitudes and the hero
    // really is passing through them, so the collection that happens is the
    // same collection that happens everywhere else in the game.
    const riding = !!(this.loop && !this.loop.pending);
    // WHAT COUNTS AS "HIM" ON THE RING.
    //
    // `playerBox` is an upright rectangle standing on his feet, and nothing in
    // the engine ever turns it. That is fine everywhere else in the game, where
    // he is the right way up — and wrong for a whole second here, because at the
    // top of the loop the box points up out through the track while the hero it
    // describes is hanging down inside it. Collecting against that box is what
    // forced the ring's coins out onto the rail, where they were half-buried in
    // the structure and read as stuck to it rather than hanging in it.
    //
    // So for the ride, and only for the ride, he is a box around his MIDDLE,
    // which turns with him. Square rather than 8x14: an oriented rectangle is
    // the honest shape and an axis-aligned one is what `overlaps` speaks, and
    // being a little generous costs nothing on a stretch where the player has no
    // control to be judged on anyway.
    const ridePbox = riding ? (() => {
      const b = loopBodyPoint(this.loop.cx, this.loop.r, this.loop.theta);
      const h = PLAYER_H / 2;
      return { x: b.x - h, y: this.groundYAt(b.x) - b.alt - h, w: PLAYER_H, h: PLAYER_H };
    })() : null;
    // Obstacles.
    for (const ob of riding ? [] : this.obstacles) {
      if (!ob.live) continue;
      ...
      if (ob.def.isLoop) {
        const box = entityBox(ob, this.entityGroundY(ob));
        // The same grounded gate the other two pads use, and it is the whole
        // answer to "what if he jumps it": the pad never sees him, the ring
        // scrolls past with its coins still on it, and the telegraph's tick
        // falls away to say so. Nothing is taken off him for it. Falling costs
        // nothing anywhere in this game, and a set piece that killed you for
        // mistiming a jump you were never told to make would be the one place
        // it did.
        if (overlaps(pbox, box) && this.player.grounded && !ob.used) {
          ob.used = true;
          ob.hitT = BOOST_HIT_TAIL;
          ob.tickT = 0.045;
          ob.firedT = BOOST_FLARE_T;
          this.loop = startLoop(ob);
          // Sampled once, here, and held for the lap: the figure is one gesture
          // in one key, and one that re-read the song halfway round would change
          // key under itself on a section boundary.
          this.loop.notes = loopClimbNotes();
          // It is a boost pad. It boosts. Given on CONTACT rather than at the
          // far end, so the hero accelerates over the last few pixels of ground
          // and the ride begins carrying it — the whole lap is then quicker than
          // the lane either side of it, which is the only way a loop reads as a
          // reward rather than as a detour you brake for.
          this.speedBoost = Math.min(1.0, this.speedBoost + LOOP.entryBoost);
          Audio.sfx('boost');
          this.score += LOOP.entryScore;
      ...
      // The JUMP sign. It is a word, not a wall: anything at all that touches it
      // knocks it over, at no cost. That is the contract — the run put it in the
      // lane uninvited, so it may not also be something the player has to deal
      // with, and a player who reads it late and runs into it must not be
      // punished for the hint they were given.
      // The JUMP sign's contract, extended to the one other thing in the lane
      // the run puts there uninvited: the CARD BOX. Half the roster cannot
      // shoot, and a relay swap can walk one of them straight into a box laid
      // for the hero before them — so running through it has to cost nothing,
      // exactly as running through the hint does. It is a cardboard box; it
      // comes apart and you keep going.
      if (ob.def.sign || ob.def.pushover) {
        if (overlaps(pbox, entityBox(ob, this.entityGroundY(ob)))) {
          // A LIT BOX IS NEVER PLOWED. The geometry says the burst lands four
          // tenths of a beat in front of the hero and this should not be
          // reachable — but a shot fired at the very late edge of the on-beat
          // window can push the fuse a whole beat out, and a box that had
          // visibly caught its round and then came apart under a boot would
          // read as a dud. It goes off instead, which is what it was going to
          // do; the only thing lost is the grid, and only for a press that was
          // already at the edge of the window.
          if (ob.burstBeat != null) { this.burstCardBox(ob); continue; }
          // `quiet` because breakObstacle plays the generic crunch, and a sign
          // is board rather than crate: the same cue a fifth higher, once.
          this.breakObstacle(ob, false, true);
          Audio.sfx('crunch', { pitch: 1.3 });
        }
        continue;
      }
      const box = entityBox(ob, this.entityGroundY(ob));
      ...
      if (ob.punted) {
        // Only the LIGHT prop can be kept up. A heavy one has been kicked the
        // other way down the road and is pulling away at 1.7x the run from the
        // frame it was hit (HEAVY_PUNT) — the hero cannot get to it, and on the
        // one frame he could, heading it back up would apply `juggleCarry` and
        // slow the thing to exactly his own speed. A barrel pacing him to the
        // horizon is the opposite of what the boot just bought.
        if (ob.def.punt !== 'heavy' && ob.alt > 0 && !this.player.grounded
            && !this.player.airJuggled && overlaps(pbox, box)) {
      ...
      //
      // The one thing it does NOT survive is a kick at the obstacle that just
      // hit him. That box already won the exchange — he ran into it standing
      // up — and letting the late slide plow it anyway turned a miss into a
      // free break, with the boot going through a crate that had already
      ...
      if (sliding && !ob.hurtPlayer && (ob.type === 'crate' || ob.type === 'qcrate')) {
        ...
        // The full smash flourish. This used to borrow the WEAPON crash — a
        // spanner projectileImpact, gold rings and all — and it was wrong twice
        // over: it is Lorenzo's metal-on-metal ring for a wooden box, and it is
        // a hit landing NEAR the crate rather than a boot going through it. Both
        // halves are the plow's own now, and both are louder than what they
        // replace, because this break is the one the player went and got.
        Audio.sfx('boxKick');
        this.plowBurst(ob, ob.x + ob.w / 2,
          this.entityGroundY(ob) - ob.alt - ob.h / 2);
        this.breakObstacle(ob, false, true);   // quiet: boxKick already spoke
        continue;
      }
      ...
      //
      // The window is the same for every puntable prop and the ARC is not (see
      // puntTuneFor): the timing is a skill the player carries from the cones
      // in one cabinet to the barrels in all of them, and what changes between
      // them is only how the thing flies once it has been hit. A barrel is the
      // interesting one, because it is rolling AT him — the read is against a
      ...
      if (sliding && !ob.hurtPlayer && ob.def.punt) {
        ...
        if (power > 0) {
          ...
          //
          // The same cue at a lower pitch for the heavier prop, and the scuffs
          // in that prop's OWN colours rather than the cone's orange — the
          // burst is meant to read as bits of the thing that just left the
      ...
      // an exception carved out ahead of the rest of the rules.
      if (ob.def.slip) this.slip(ob);
      else this.takeHit(null, false, ob.type);
      break;
    }
    // Pickups.
    //
  ...
  onPickup(p) {
    ...
    if (p.def.coin) {
      // A coin that belongs to the ring he is currently riding. Counted rather
      // than scored differently — it pays out like any other coin — because the
      // only thing the count decides is whether the lap was completed, which is
  ...
  // ------------------------------------------------------------------ damage
  // THE PRATFALL. A banana peel costs a battery cell like any other hazard and
  // then, unlike any other hazard, takes the next beat of his run away too.
  //
  // The damage goes through takeHit unchanged, which is what keeps the peel
  // honest: UNPEELABLE deflects it, a shield eats it, i-frames ghost it, Ray
  // M'N's grace catches the fatal one, and the crash test logs it. What is NOT
  // unchanged is that the slip only follows a hit that actually LANDED —
  // `damageTaken` is the one counter every one of those escape hatches skips
  // and the real hit always increments, so it is the honest question to ask.
  // Slipping through an absorbed hit would have had the hero pratfall while the
  // shield was busy telling him he was fine.
  //
  // A death is not a pratfall either: die() has already taken the run, and a
  // tumbling hero over a death screen is two endings playing at once.
  slip(ob) {
    const before = this.damageTaken;
    this.takeHit(null, false, ob.type);
    if (this.damageTaken === before || this.battery <= 0) return;
    this.player.slipT = SLIP_T;
    // The control penalty is the EXISTING stumble — slower, and no jump until it
    // clears. Reusing it rather than inventing a second one means the peel is
    // tuned against a feel the game already has, and that a hero mid-slip is
    ...
    Audio.sfx('slideWhistle', { dur: SLIP_T, pitch: 0.7 });
    this.floatText('BANANA. CLASSIC.', '#f6d33c');
  ...
  takeHit(msg, isPit = false, src = null, pit = null) {
    ...
    // circle would win. Left airborne at whatever height the ring had him at,
    // which is a fall, and a fall has never hurt anybody here.
    if (this.loop) {
      const alt = this.player.y;
      this.loop = null;
      this.player.y = alt;
  ...
  // GOING UNDER. A pit death is the one death in this game that has something
  // to show, so it is the one that gets a beat before the checkpoint takes over.
  //
  // Every other death is a hit: it happens on one frame, the reaction plays, and
  // half a second later the run picks up at the last checkpoint. A hole is not
  // an event, it is a JOURNEY — over the edge, down the shaft with a face on,
  // into whatever is at the bottom, and under. Cutting away half a second in
  // means cutting away at the exact moment the player finds out what the hole
  // was for, and they never see the thing that killed them.
  //
  // Three phases, and the hero is on screen for all of them (see deadHold):
  // the fall, the sink, and an empty hole. The empty hole is not padding — it
  // is the beat that says he is not coming back out, and it is the difference
  ...
  // death owns its own clock because it has three phases to get through.
  deadHold() {
    return this.pitDeath ? PIT_DEATH_HOLD : 0.5;
  }
  ...
  floatText(text, color, { solid = false, keep = false, hold = 0, wx = null, base = floatBaseY() } = {}) {
    ...
    // Floaties use the same portrait ink budget as the HUD: the old 1.24
    // multiplier was only enough to undo the phone's downscale and still
  ...
  drawFrame(ctx, renderAlpha = 0) {
    ...
    // world x and the camera — and this expression did not consult it, so the
    // pit-death carry that method reports was invisible: the sprite stayed
    // welded to PLAYER_X and dropped down the near wall like a lift while every
    // non-drawing system believed he was arcing into the hole.
    ...
    const portraitWorldXOffset = this.portraitWorldXOffset();
    const portraitXOffset = portraitWorldXOffset * z;
    const portraitFrameActive = !!(this.portraitGameplay && isPhonePortraitPresentation());
    // Moving the presentation lane left makes more of the authored world visible
    // on the right. Extend the render band by that exact world-space amount so
    // ground, routes and entities continue painting all the way to the phone edge.
    // The extra band belongs only to the phone portrait frame; shipped
    // landscape framing keeps its existing width and culling behavior.
    const visibleWorldW = portraitRenderViewWidth(z,
      portraitFrameActive ? portraitWorldXOffset : 0);
    const portraitRenderViewW = portraitFrameActive ? visibleWorldW : null;
    ctx.save();
    ...
    // The character anchor is a whole-scene presentation shift. Apply it
    // after the optional backdrop scale so mountains, clouds and the playable
    // lane remain in the same horizontal frame rather than being painted twice
    // at different anchors.
    if (Math.abs(portraitXOffset) > 0.0001) ctx.translate(portraitXOffset, 0);
    const backgroundCoverage = portraitFrameActive
      ? portraitBackgroundCoverage(portraitXOffset, bgZoom) : null;
    ...
    // the one gameplay fact it gets — HOW IT IS GOING, as a single 0..1 scalar
    // plus a streak flag. It still may not see chart events, player actions,
    // obstacle state or position: `form` is a summary the run computes, not a
    // window onto the lane, and the rooftop share price is the only thing that
    // spends it. An invalid clock selects the pack's stable idle frame while
    // the lane is waiting to re-anchor.
    //
    // `audio` is the analyser's own object, which Audio owns and REUSES every
    // frame — the pack reads it inside bg() and must never retain it. It is
    // the same call the beat ribbon makes (hud.js), and musicAnalysis() is
    // memoised against the audio clock precisely so two callers in one frame
    // share one honest read: the readout is a STEP (it advances the smoothers
    // and drains the drum queue), so an unguarded second call would have
    // halved the ribbon's own hit envelope.
    //
    ...
    ctx.save();
    applyWorld(ctx, z, pan, floorY, portraitXOffset);

    // Ground line + gaps.
    //
    // WHERE THE LANE STOPS BEING GROUND. A tunnel is not a hole cut in the lane
    // — the path goes down, and what is over the top of it is a long island —
    // so along that stretch the lane has no body, no apron and no surface line
    // of its own. Both terrain painters and the style pack have to be told, and
    // they have to be told BEFORE they paint: by the time the island is drawn
    // the green is already over the hills and there is nothing to paint it back
    // to. Empty on every stage without a tunnel in front of the camera, which
    // is nearly all of them.
    const laneCuts = this.routes.length ? tunnelOverhangs(this.routes) : [];
    // How deep the hill is grass before it becomes dirt: the depth of the
    // deepest road under it, so the band under the lane and the ground the
    ...
    // visibleWorldW is the world this frame actually shows, including the
    // portrait runway exposed when the presentation lane is shifted left. It is
    ...
    //
    // ...and `ownPitFills` is what keeps the ninth from painting teeth at all.
    // A pack that cuts its own material into every break it draws (the LCD
    // city's cogwheels) has already answered the question this pass exists to
    ...
    if (this.routes.length) {
      ...
      // that they paint nothing, so the moment the camera drops to follow a hero
    ...
    const cullRight = cam + visibleWorldW + LOOP_CULL_MARGIN;
    ...
    const onScreen = (e) => {
      const visible = entityInRenderBand(e, cam, visibleWorldW);
    ...
    // behind a crate should be behind the crate. The ring is not that kind of
    // object. It is a piece of ROAD, its coins ride on its surface, and painting
    // it afterwards buried all eight of them under the band — the reward for the
    // set piece was invisible from the approach, which is the one moment it has
    // to sell itself. Scenery goes behind the things standing on it.
    //
    // No smear pass either: the ring is 92px of static structure and would cost
    // several full redraws a frame to fuse motion it does not have.
    for (const ob of this.obstacles) {
    ...
    for (const pr of this.projectiles) {
      ...
      if (pr.type === 'enemyShot') {
        ...
        const shotPal = (HERO_SPRITES[pr.contactHero] || {}).pal;
        const coral = shotPal && shotPal.ki === '#f2767f';
        drawPellet(ctx, x + 3, y + 2, {
          size: pr.size,
    ...
    const drawHero = () => this.rhythmHeroVisible() && drawHeroSprite(ctx, this.player, this.relay.current, heroT, cam, carryFuse,
      { mirror: this.mirror, screenX: heroScreenX, zoom: z, pan, floorY, xOffset: portraitXOffset, specialOrbAlpha: orbAlpha,
        ...
        // Running the wall. `lean` rotates the whole figure about the feet, so
        // the pose that carries him round the ring is the ordinary run cycle
        // turned to the tangent — upright at the bottom, sideways up the near
        // wall, inverted over the top. Negative because the canvas turns
        // clockwise and the ride goes the other way, and it passes through
        // exactly -2pi as the lap closes, which is upright again: the ride ends
        // with no rotation to unwind and no blend to hide a snap.
        //
        // Everything else in the patch is cleared for the same reason the
    ...
    // Ground is painted through the world transform above; this final screen
    // edge pass darkens only the last small band of the frame, where portrait
    // otherwise leaves a large flat slab beneath the playable route.
    drawGroundEdgeGradient(ctx, this.style);
    ...
    if (this.mission.type === 'blackout') {
      ...
      const px = (hsx + 6) * z + portraitXOffset;
    ...
    const drawChatter = (d) => {
      ...
      // reason.
      const heroX = (heroScreenX + portraitWorldXOffset) * z;
      ...
      const markerX = (!this.overtime && Number.isFinite(this.totalDist))
        ? this.finishWorldX() - cam + portraitWorldXOffset
        : null;
      ...
      if (this.speech && !this.paused) {
        ...
        // to press.
        // Portrait HUD geometry is resolved from the active safe frame.  A
        // rotation, notch change, or browser toolbar resize can revise that
        // frame while the same line is still alive; refresh the channel then
        // so its wrap and portrait card do not keep stale landscape bounds.
        const frameRevision = presentationFrame().revision;
        if (!this.speech.chan || this.speech.chanRevision !== frameRevision) {
          this.speech.chan = speechChannel(this);
          this.speech.chanRevision = frameRevision;
        }
  ...
  drawPortraitPaused(ctx) {
    ...
    buttons.forEach((b, i) => {
      ...
      drawPanel(ctx, b.x, b.y, b.w, b.h, 8, 'rgba(11,11,20,0.88)',
  ...
  drawDebug(ctx) {
    ...
    ctx.save();
    applyWorld(ctx, this.camZoom, this.camPan, this.camFloorY,
      this.portraitWorldXOffset() * this.camZoom);

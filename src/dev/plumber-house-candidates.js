// PLUMBER PANIC — THE HOUSE ON THE NEAR HILLS (BAKE-OFF, not wired into the game).
// Peter, 24 Sep 2026: "Better house in plumber (as good as the barnhouse)".
//
// SETTLED 24 Sep 2026: B, C, E and F SHIP (stylePacks/plumberHouses.js), one house per
// house cell, which one by stage (plumberHouseTypeFor in stylePacks/index.js). Card A is
// the house they replaced — a 30x29 box, triangle roof, round window and door at 0.86,
// with a bush on its right and a fence on its left — kept as a faithful reproduction.
// The bar is drawPlumberBarn (stylePacks/plumberLandmarks.js): a baked, cut-paper body
// (deep lift, contact edge, flat fill, fibre grain, white rim) with live extras on top.
//
// Every candidate here is built THAT way: the house and its garden are one baked body
// in the landmarks' cut()/tone() finish, and only what moves (smoke, laundry, a cat's
// tail, a gate, a dog) is drawn live. Each owns its whole vignette — the house and its
// garden — as the shipped houses do.
//
// PLACEMENT is the shipped slot, exactly: drawPlumberHouseScene() runs the real pack
// backdrop with the pack's ctx.__mashPlumberSceneryOverride seam set, so
// drawPlumberScenery() hands each house slot to the candidate in the slot's own frame
// (foot on the base the pack computed from the ridge, near layer's transform) instead of
// painting the shipped house. Every card, A included, is painted that way.
//
// Candidate painters take (ctx, t, paper) and draw in local units with the vignette's
// foot centre at (0, 0); `S` is the scale they are placed at (1 = one unit per frame px).
import { GROUND_Y, VIEW_W, ZOOM, applyWorld } from '../engine/camera.js';
import { getStylePack, __testing as SCENERY } from '../engine/stylePacks/index.js';
import { drawPlumberBarn } from '../engine/stylePacks/plumberLandmarks.js';
...
// A copy of plumberLandmarks.js's private cut()/tone()/baked() so the candidates wear
// the barn's finish to the pixel (that module does not export them).
let PAPER = true;
...
function deviceScale(ctx) {
  ...
  return Math.max(1, Math.min(8, Math.round(s * 4) / 4));
}
...
// ====================================================================== A — NOW
// plumberScenerySprite('house') is private to stylePacks/index.js, so the close-up is
// a line-for-line REPRODUCTION of it (and of the cluster's bush and fence, at the
// cluster's own offsets and scales). The in-scene card is the real pack, untouched.
const NOWP = {
  house: '#d0c39f', roof: '#ad7969', roofCool: '#7e959a', window: '#697970', timber: '#c9b78b',
  timberShade: '#a18b65', bush: '#4d8e54', bushLight: '#70a15b', petal: '#eee4bf', petalPink: '#cf8d9c',
};
// plumberSceneryFill's subtle recipe is the landmarks' cut() at lift 1, rim 1.15.
const nowFill = (ctx, fn, color, rim = true) => cut(ctx, fn, color, { lift: 1, rim: rim ? 1.15 : false });
function nowRect(ctx, x, y, w, h, color, paper = true) {
  if (paper) nowFill(ctx, (c) => c.rect(x, y, w, h), color, false);
  else { ctx.fillStyle = color; ctx.fillRect(x, y, w, h); }
}
function nowHouseSprite(ctx, variant = 0) {
  const p = NOWP;
  nowRect(ctx, 4, 12, 22, 17, p.house);
  nowFill(ctx, (c) => { c.moveTo(2, 13); c.lineTo(15, 2); c.lineTo(28, 13); c.closePath(); }, variant % 2 ? p.roofCool : p.roof);
  nowRect(ctx, 20.5, 5, 3, 7, p.roof);
  nowFill(ctx, (c) => c.ellipse(15, 18, 2.5, 2.5, 0, 0, TAU), p.window);
  ctx.strokeStyle = p.house; ctx.lineWidth = 0.8; ctx.beginPath(); ctx.arc(15, 18, 3.1, 0, TAU); ctx.stroke();
  nowRect(ctx, 7, 22, 4, 7, p.timberShade, false);
}
function nowBushSprite(ctx, variant = 0) {
  const p = NOWP; const height = 11;
  nowFill(ctx, (c) => {
    c.moveTo(1, height); c.lineTo(1, 7);
    c.arc(5, 7, 4, Math.PI, TAU); c.arc(11, 5.5, 5.5, Math.PI, TAU);
    c.arc(17, 7, 4.5, Math.PI, TAU); c.lineTo(21, height); c.closePath();
  }, p.bush);
  nowFill(ctx, (c) => { c.arc(11, 5.8, 3.5, Math.PI, TAU); c.closePath(); }, p.bushLight, false);
  if (variant % 3 === 1) {
    nowFill(ctx, (c) => c.ellipse(6, 5.1, 0.8, 0.55, 0, 0, TAU), p.petalPink);
    nowFill(ctx, (c) => c.ellipse(17, 6, 0.75, 0.5, 0, 0, TAU), p.petal);
  }
}
function nowFenceSprite(ctx) {
  const p = NOWP; const height = 16;
  ctx.fillStyle = p.timberShade;
  ctx.beginPath(); ctx.moveTo(2, 6); ctx.lineTo(31, 4.5); ctx.lineTo(31, 6.5); ctx.lineTo(2, 8); ctx.closePath(); ctx.fill();
  ctx.beginPath(); ctx.moveTo(3, 10); ctx.lineTo(30, 9); ctx.lineTo(30, 11); ctx.lineTo(3, 12); ctx.closePath(); ctx.fill();
  for (const [x, lean] of [[3, -0.5], [17, 0.4], [30, -0.35]]) {
    nowFill(ctx, (c) => { c.moveTo(x - 1 + lean, height); c.lineTo(x - 0.75, 2.5); c.lineTo(x + 1.35, 2.8); c.lineTo(x + 1 + lean, height); c.closePath(); }, p.timber, false);
    ctx.fillStyle = p.timberShade; ctx.fillRect(x + 0.7, 3.3, 0.6, 11);
  }
}
// The cluster: house at 0.86, bush at +17 and 0.72, fence at -20 and 0.55. In game each is
// seated on the crest at its OWN x (house +28, bush +15, fence +18 below it) and clipped
// to the hill's face, so on the close-up's flat floor all three simply stand on it.
function nowVignette(ctx) {
  const put = (dx, lift, s, w, h, fn) => {
    ctx.save(); ctx.translate(dx, -lift); ctx.scale(s, s); ctx.translate(-w / 2, -h); fn(); ctx.restore();
  };
  put(0, 0, 0.86, 30, 29, () => nowHouseSprite(ctx, 0));
  put(17, 0, 0.72, 22, 11, () => nowBushSprite(ctx, 0));
  put(-20, 0, 0.55, 34, 16, () => nowFenceSprite(ctx));
}
...
function roseCottage(ctx, t) {
  baked(ctx, 'plh-rose', -26, -40, 52, 46, roseBody);
...
function farmhouse(ctx, t) {
  baked(ctx, 'plh-farm', -30, -40, 57, 46, farmBody);
...
// ====================================================================== D — timbered cottage and cat
const TUDOR = {
  plaster: '#f1e8d0', plasterShade: '#dccfb2', beam: '#4e3c2f', tile: '#b3664f', tileShade: '#914f3e',
  tileLine: 'rgba(90,40,28,0.4)', brick: '#a8584a', brickShade: '#87463b', door: '#6d4c33', doorShade: '#573b27',
  box: '#7a5a3e', gera: '#d8453a', cat: '#e0913f', catShade: '#bf7229', catStripe: 'rgba(150,80,24,0.7)', catLight: '#f6d7a8',
};
function tudorBody(ctx) {
  const P = TUDOR;
  lawn(ctx, -24, 24);
  // The chimney stack, tall and clustered, behind the roof.
  const ch = cut(ctx, (c) => c.rect(9.2, -40, 4, 16), P.brick, { lift: 0.8, rim: 0.6 });
  tone(ctx, ch, (c) => c.rect(11.6, -42, 2, 20), P.brickShade);
  inked(ctx, ch, 'rgba(70,30,24,0.4)', 0.28, (c) => { for (let y = -38.6; y < -26; y += 1.4) { c.moveTo(9.2, y); c.lineTo(13.2, y); } c.moveTo(9.2, -35); c.lineTo(13.2, -33); c.moveTo(13.2, -35); c.lineTo(9.2, -33); });
  cut(ctx, (c) => c.rect(8.6, -41.2, 5.2, 1.4), P.brickShade, { lift: 0.3, rim: 0.4 });
  cut(ctx, (c) => { c.rect(9.5, -43, 1.3, 1.9); c.rect(11.6, -42.8, 1.3, 1.7); }, '#9a5238', { lift: 0.3, rim: 0.35 });
  // The main roof's front pitch, steep, in peg tiles.
  const roof = cut(ctx, (c) => { c.moveTo(-15.5, -20.4); c.lineTo(-11, -32.2); c.lineTo(11, -32.2); c.lineTo(15.5, -20.4); c.closePath(); }, P.tile, { lift: 1.1 });
  tone(ctx, roof, (c) => { c.moveTo(9, -34); c.lineTo(12, -34); c.lineTo(17, -19); c.lineTo(11, -19); c.closePath(); }, P.tileShade);
  inked(ctx, roof, P.tileLine, 0.3, (c) => { for (let y = -30.8; y < -20.4; y += 1.3) { c.moveTo(-16, y); c.lineTo(16, y); } });
  // The upper storey, jettied out over the ground floor.
  const up = cut(ctx, (c) => c.rect(-13.6, -21, 27.2, 10), P.plaster, { lift: 1.2 });
  tone(ctx, up, (c) => c.rect(-13.6, -21, 27.2, 1.4), P.plasterShade);
  const ground = cut(ctx, (c) => c.rect(-12, -11, 24, 11), P.plaster, { lift: 1 });
  tone(ctx, ground, (c) => c.rect(-12, -11, 24, 2), P.plasterShade);
  // The front gable over the middle, its face timbered.
  const gable = cut(ctx, (c) => { c.moveTo(-7.4, -21); c.lineTo(0, -34.6); c.lineTo(7.4, -21); c.closePath(); }, P.plaster, { lift: 1 });
  tone(ctx, gable, (c) => { c.moveTo(0, -35); c.lineTo(8, -21); c.lineTo(3, -21); c.closePath(); }, P.plasterShade);
  line(ctx, P.beam, 0.7, (c) => {
    c.moveTo(0, -33.6); c.lineTo(0, -21); c.moveTo(-4.4, -26.6); c.lineTo(4.4, -26.6);
    c.moveTo(-5.6, -21.4); c.lineTo(0, -26.6); c.moveTo(5.6, -21.4); c.lineTo(0, -26.6);
    c.moveTo(-2.4, -26.6); c.lineTo(-2.4, -21); c.moveTo(2.4, -26.6); c.lineTo(2.4, -21);
  });
  // Bargeboards down the gable's edges.
  const barge = cut(ctx, (c) => { c.moveTo(-8.6, -20.4); c.lineTo(0, -35.8); c.lineTo(8.6, -20.4); c.lineTo(7.2, -20.4); c.lineTo(0, -33.4); c.lineTo(-7.2, -20.4); c.closePath(); }, P.tile, { lift: 0.8, rim: 0.6 });
  tone(ctx, barge, (c) => c.rect(0, -37, 10, 18), P.tileShade);
  // Timber framing: close studding upstairs, posts and short studs below.
  line(ctx, P.beam, 0.75, (c) => {
    for (let x = -12.8; x <= 12.9; x += 2.14) {
      if ((x > -10.6 && x < -5) || (x > 5 && x < 10.6)) continue;
      c.moveTo(x, -20.4); c.lineTo(x, -11.6);
    }
    c.moveTo(-11.5, -10.4); c.lineTo(-11.5, 0); c.moveTo(11.5, -10.4); c.lineTo(11.5, 0);
    c.moveTo(-2.9, -10.4); c.lineTo(-2.9, 0); c.moveTo(2.9, -10.4); c.lineTo(2.9, 0);
    for (const x of [-9.2, -6.9, -4.8, 4.8, 6.9, 9.2]) { c.moveTo(x, -3.9); c.lineTo(x, 0); }
    c.moveTo(-11.5, -3.9); c.lineTo(-2.9, -3.9); c.moveTo(2.9, -3.9); c.lineTo(11.5, -3.9);
    c.moveTo(-11.5, -9.4); c.lineTo(-9.8, -10.6); c.moveTo(11.5, -9.4); c.lineTo(9.8, -10.6);
  });
  cut(ctx, (c) => c.rect(-14, -21.4, 28, 1.1), P.beam, { lift: 0.4, rim: false });
  cut(ctx, (c) => c.rect(-14, -11.8, 28, 1.3), P.beam, { lift: 0.6, rim: 0.4 });
  cut(ctx, (c) => c.rect(-12.2, -0.9, 24.4, 0.9), P.beam, { lift: 0.3, rim: false });
  // Leaded casements.
  casement(ctx, -10.4, -19.2, 5.2, 4.2, { bars: 'leaded', frame: P.beam, fw: 0.55, sill: false });
  casement(ctx, 5.2, -19.2, 5.2, 4.2, { bars: 'leaded', frame: P.beam, fw: 0.55, sill: false });
  casement(ctx, -2.2, -32, 4.4, 3, { bars: 'leaded', frame: P.beam, fw: 0.45, sill: false });
  casement(ctx, -10.3, -9, 5.8, 4.4, { bars: 'leaded', frame: P.beam, fw: 0.55, sill: false });
  casement(ctx, 4.5, -9, 5.8, 4.4, { bars: 'leaded', frame: P.beam, fw: 0.55, sill: false });
  // The cat's sill.
  cut(ctx, (c) => c.rect(4, -4.7, 6.8, 0.9), P.beam, { lift: 0.4, rim: 0.4 });
  // A window box of geraniums under the left window.
  const box = cut(ctx, (c) => c.rect(-10.9, -4.7, 6.8, 1.6), P.box, { lift: 0.5, rim: 0.45 });
  tone(ctx, box, (c) => c.rect(-10.9, -3.6, 6.8, 0.5), '#5e4430');
  const leaves = cut(ctx, (c) => { for (let x = -10.4; x < -4.2; x += 1.2) { c.moveTo(x + 0.9, -5); c.arc(x, -5, 0.9, 0, TAU); } }, G.leaf, { lift: 0.3, rim: 0.35, rimUnder: true });
  tone(ctx, leaves, (c) => c.rect(-11, -6.2, 8, 0.8), G.leafLight);
  for (let x = -10; x < -4.4; x += 1.5) dot(ctx, x + 0.3, -5.9 + ((x * 10) % 2 ? 0.3 : 0), 0.62, P.gera);
  // An oak door, arched and studded, and a boot scraper step.
  const door = cut(ctx, (c) => { c.moveTo(-2.2, 0); c.lineTo(-2.2, -7.4); c.quadraticCurveTo(0, -9.8, 2.2, -7.4); c.lineTo(2.2, 0); c.closePath(); }, P.door, { lift: 0.35, rim: 0.5 });
  tone(ctx, door, (c) => c.rect(0.6, -10, 2, 10), P.doorShade);
  inked(ctx, door, 'rgba(30,18,10,0.45)', 0.25, (c) => { c.moveTo(-0.7, -8.6); c.lineTo(-0.7, 0); c.moveTo(0.7, -8.6); c.lineTo(0.7, 0); c.moveTo(-2.2, -5.4); c.lineTo(2.2, -5.4); c.moveTo(-2.2, -2); c.lineTo(2.2, -2); });
  dot(ctx, 1.5, -3.9, 0.32, '#2a2320');
  cut(ctx, (c) => c.rect(-2.9, -0.6, 5.8, 1.1), G.stone, { lift: 0.3, rim: 0.4 });
  // A clipped box ball in a pot either side, and lupins at the corner.
  for (const bx of [-14.6, 14.8]) {
    cut(ctx, (c) => { c.moveTo(bx - 1.2, 0.4); c.lineTo(bx - 1.5, -1.6); c.lineTo(bx + 1.5, -1.6); c.lineTo(bx + 1.2, 0.4); c.closePath(); }, '#b8704f', { lift: 0.4, rim: 0.4 });
    const ball = cut(ctx, (c) => c.arc(bx, -3.6, 2.2, 0, TAU), '#3f7f48', { lift: 0.5, rim: 0.5 });
    tone(ctx, ball, (c) => c.arc(bx - 0.7, -4.3, 1.2, 0, TAU), '#579153');
  }
  for (const [lx, col] of [[-18.5, '#8e7fc2'], [-17.2, '#e2a3c0'], [17.8, '#8e7fc2'], [19, '#f1d8e2']]) {
    line(ctx, G.leaf, 0.45, (c) => { c.moveTo(lx, 0.6); c.lineTo(lx, -5); });
    const spike = cut(ctx, (c) => { c.moveTo(lx - 0.75, -3.6); c.quadraticCurveTo(lx - 0.6, -8, lx, -9); c.quadraticCurveTo(lx + 0.6, -8, lx + 0.75, -3.6); c.closePath(); }, col, { lift: 0.3, rim: 0.35 });
    inked(ctx, spike, 'rgba(255,255,255,0.35)', 0.25, (c) => { for (let y = -8; y < -3.6; y += 0.8) { c.moveTo(lx - 0.8, y); c.lineTo(lx + 0.8, y + 0.4); } });
  }
  tufts(ctx, [[-22, 2.8], [-6, 1.6], [7, 1.6], [21.5, 2.8]]);
}
// The ginger cat on the sill: sits, swishes its tail, looks about, blinks.
function tudorCat(ctx, t) {
  const P = TUDOR;
  ctx.save();
  ctx.translate(8.2, -4.7);
  ctx.scale(1.15, 1.15);
  // Tail hangs over the sill's edge and swishes at the tip.
  const sw = Math.sin(t * 2.1) * 0.9 + Math.sin(t * 5.3) * 0.2;
  const tail = pathOf((c) => { c.moveTo(-1.2, -0.4); c.bezierCurveTo(-2.6, 0.2, -2.2, 2.4, -2.4 + sw * 0.5, 3.2); c.bezierCurveTo(-2.5 + sw, 4.1, -1.6 + sw * 1.4, 4.4, -1.3 + sw * 1.2, 3.7); });
  ctx.save(); ctx.lineWidth = 0.95; ctx.lineCap = 'round'; ctx.strokeStyle = P.catShade; ctx.stroke(tail); ctx.restore();
  // Body: a sitting loaf, haunch towards us.
  const body = cut(ctx, (c) => { c.moveTo(-1.8, 0); c.bezierCurveTo(-2.1, -2, -1.2, -3.8, 0.1, -3.9); c.bezierCurveTo(1.3, -3.8, 1.9, -2.2, 1.8, 0); c.closePath(); }, P.cat, { lift: 0.4, rim: 0.45 });
  tone(ctx, body, (c) => c.ellipse(-0.6, -0.9, 1.4, 1.1, 0, 0, TAU), P.catShade);
  tone(ctx, body, (c) => c.ellipse(0.9, -2.2, 0.7, 1.3, 0, 0, TAU), P.catLight);
  inked(ctx, body, P.catStripe, 0.25, (c) => { c.moveTo(-1.6, -1.6); c.lineTo(-0.8, -1.8); c.moveTo(-1.5, -2.5); c.lineTo(-0.6, -2.6); });
  // Head: looks about now and then, blinks.
  const look = Math.sin(t * 0.45) > 0.55 ? -0.45 : Math.sin(t * 0.45) < -0.7 ? 0.35 : 0;
  const hx = 0.2 + look, hy = -4.6;
  const head = cut(ctx, (c) => {
    c.moveTo(hx - 1.35, hy + 0.3); c.lineTo(hx - 1.2, hy - 1.6); c.lineTo(hx - 0.45, hy - 0.95); c.lineTo(hx + 0.45, hy - 0.95); c.lineTo(hx + 1.2, hy - 1.6); c.lineTo(hx + 1.35, hy + 0.3);
    c.bezierCurveTo(hx + 1.3, hy + 1.4, hx - 1.3, hy + 1.4, hx - 1.35, hy + 0.3); c.closePath();
  }, P.cat, { lift: 0.35, rim: 0.4 });
  tone(ctx, head, (c) => c.ellipse(hx, hy + 0.85, 0.75, 0.45, 0, 0, TAU), P.catLight);
  inked(ctx, head, P.catStripe, 0.22, (c) => { c.moveTo(hx - 0.3, hy - 0.9); c.lineTo(hx - 0.2, hy - 0.4); c.moveTo(hx + 0.3, hy - 0.9); c.lineTo(hx + 0.2, hy - 0.4); });
  const blink = (t % 3.7) < 0.14;
  if (blink) line(ctx, '#3a2a1c', 0.2, (c) => { c.moveTo(hx - 0.75, hy + 0.1); c.lineTo(hx - 0.25, hy + 0.1); c.moveTo(hx + 0.25, hy + 0.1); c.lineTo(hx + 0.75, hy + 0.1); });
  else { dot(ctx, hx - 0.5 + look * 0.3, hy + 0.05, 0.28, '#6f8a3a'); dot(ctx, hx + 0.5 + look * 0.3, hy + 0.05, 0.28, '#6f8a3a'); dot(ctx, hx - 0.5 + look * 0.4, hy + 0.05, 0.12, '#1a1410'); dot(ctx, hx + 0.5 + look * 0.4, hy + 0.05, 0.12, '#1a1410'); }
  dot(ctx, hx + look * 0.3, hy + 0.6, 0.16, '#c9737a');
  ctx.restore();
}
function tudorCottage(ctx, t) {
  baked(ctx, 'plh-tudor', -26, -46, 52, 52, tudorBody);
  smoke(ctx, t, 10.2, -43.6, { seed: 0.55, n: 4, rise: 15 });
  smoke(ctx, t, 12.3, -43.4, { seed: 0.05, n: 4, rise: 14, r1: 2.4 });
  tudorCat(ctx, t);
}
...
function shepherdsHut(ctx, t) {
  baked(ctx, 'plh-hut', -27, -34, 54, 40, hutBody);
...
// The picket fence along the front, with the gate swinging on its hinge in the breeze.
function pinkFence(ctx, t) {
  const P = PINK;
...
function pinkCottage(ctx, t) {
  baked(ctx, 'plh-pink', -27, -36, 54, 42, pinkBody);
...
// ====================================================================== the list
const barnSeat = { near: () => 0, far: () => 0 };
export const PLUMBER_HOUSE_CANDIDATES = [
  { id: 'now', letter: 'A', name: 'WAS — the old house',
    note: 'What shipped before 24 Sep: a box, a triangle roof, a round window and a door, with a bush and a fence beside it. A faithful reproduction, in the scene and close up; the pack no longer draws it.',
    S: 1, paint: (ctx) => nowVignette(ctx), extent: [-28, -30, 56, 34] },
  { id: 'bar', letter: 'BAR', name: 'BAR — the barn',
    note: 'The bar Peter set: the real drawPlumberBarn, shown for reference only. The scene card is plumber-1 at the barn, where it really stands; the close-up is at the same zoom as every house.',
    S: 1, paint: (ctx, t, paper) => drawPlumberBarn(ctx, t, 0, barnSeat, paper), extent: [-32, -68, 80, 84] },
  { id: 'rose', letter: 'B', name: 'Rose cottage · SHIPS', ships: true,
    note: 'A whitewashed cottage under a deep thatch with an eyebrow over the attic window, roses round the door and hollyhocks at the gable. Smoke from the chimney and a butterfly at the roses; the thatch gives it the barn\'s one bold shape.',
    S: 1, paint: roseCottage, extent: [-26, -52, 52, 58] },
  { id: 'farm', letter: 'C', name: 'Stone farmhouse, washing out · SHIPS', ships: true,
    note: 'A two-storey stone farmhouse under slate, lamplit downstairs, someone going room to room upstairs; washing flapping on the line beside it. The laundry is the movement you catch from the lane.',
    S: 1, paint: farmhouse, extent: [-30, -56, 57, 62] },
  { id: 'tudor', letter: 'D', name: 'Timbered cottage and the cat',
    note: 'Black-and-white timbering with a jettied upper floor, a front gable and a tall twin-pot stack; a ginger cat on the sill swishing its tail. The busiest pattern here, so it reads as \'a house\' from the furthest away.',
    S: 1, paint: tudorCottage, extent: [-26, -60, 52, 66] },
  { id: 'hut', letter: 'E', name: 'Shepherd\'s hut · SHIPS', ships: true,
    note: 'A green corrugated hut on cast-iron wheels, curved tin roof, stovepipe smoking, stable door open on lamplight, a crook and a log pile, a lamb grazing. Belongs with the flock; smaller and lower than a house, so it sits on the hill rather than on it.',
    S: 1, paint: shepherdsHut, extent: [-27, -48, 54, 54] },
  { id: 'gate', letter: 'F', name: 'Cottage, gate, dog and hens · SHIPS', ships: true,
    note: 'A pink-washed cottage with pantiles and dormers behind a white picket fence; the gate swings in the breeze, a Labrador sits outside it wagging, hens peck, sunflowers nod. The most life, the most small parts.',
    S: 1, paint: pinkCottage, extent: [-27, -50, 54, 56] },
];

// ------------------------------------------------------------ drawing a candidate
export function drawPlumberHouseCandidate(ctx, t, cand, x, baseY, paper = true) {
  ctx.save();
  ctx.translate(x, baseY);
  ctx.scale(cand.S, cand.S);
  withFinish(paper, () => cand.paint(ctx, t, paper));
  ctx.restore();
}

// ------------------------------------------------------------ the scene
let cab = null, pack = null;
function scenePack() {
  if (!pack) { cab = CABINETS.find((c) => c.id === 'plumber'); pack = getStylePack(cab.style, {}); }
  return { cab, pack };
}
// The pack's backdrop with every house slot painted by `cand`, through the pack's
// ctx.__mashPlumberSceneryOverride seam: drawPlumberScenery calls it in the slot's own
// frame (foot at the origin, the pack's scale). Returns how many slots it painted.
function backdropWithCandidate(ctx, t, cand, camX, totalDist, scene) {
  const { cab: c, pack: p } = scenePack();
  let painted = 0;
  const prev = ctx.__mashPlumberSceneryOverride;
  ctx.__mashPlumberSceneryOverride = (g, prop, tt) => {
    if (prop.kind !== 'house') return false;
    g.save();
    g.scale(cand.S, cand.S);
    withFinish(true, () => cand.paint(g, tt, true));
    g.restore();
    painted++;
    return true;
  };
  try { p.bg(ctx, t, camX, c, totalDist, scene); } finally {
    if (prev === undefined) delete ctx.__mashPlumberSceneryOverride;
    else ctx.__mashPlumberSceneryOverride = prev;
  }
  return painted;
}
// The camera that puts the first house of the level at screen x `at`. The near
// layer travels camX * 0.35 * ZOOM; the house owns one cell in 48 (122px apart).
let houseCell = null;
function firstHouseCell() {
  if (houseCell != null) return houseCell;
  for (let k = 1; k < 400; k++) if (SCENERY.plumberSceneryClusterForCell(k) === 'house') { houseCell = k; break; }
  return houseCell;
}
export function plumberHouseCamX(at = 300) {
  return (firstHouseCell() * SCENERY.PLUMBER_SCENERY_SPACING - at) / (0.35 * ZOOM);
}
const BARN_CAM = 700;   // PLUMBER_BARN_AT_PX: plumber-1's barn arrives mid-picture here
function heroPose(t) {
  return {
...
/**
 * The 480x270 plumber frame at the game's own camera, the candidate standing where the
 * shipped house stands (for 'bar', plumber-1 at the barn). The camera holds still so
 * the house stays in the picture; the hero runs.
 */
export function drawPlumberHouseScene(ctx, t, cand, { at = 300, hero = 'lorenzo' } = {}) {
  const { cab: c, pack: p } = scenePack();
  let camX;
  if (cand.id === 'bar') {
    camX = BARN_CAM;
    p.bg(ctx, t, camX, c, 1000, { stageIndex: 1 });
  } else {
    camX = plumberHouseCamX(at);
    backdropWithCandidate(ctx, t, cand, camX, Infinity, { stageIndex: 2 });
  }
  ctx.save();
  applyWorld(ctx, ZOOM, 0, GROUND_Y);
  p.ground(ctx, camX, c, [], [], camX, VIEW_W);
  drawToon(ctx, hero, heroPose(t), PLAYER_X, GROUND_Y, HERO_DRAW_H);
  ctx.restore();
  if (p.post) p.post(ctx, t);
  if (p.weather) p.weather(ctx, t);
}
/**
 * A candidate alone, `z` times over (default 3, the same for all so sizes compare
 * honestly), feet on the near hill's green with the plumber sky behind.
 */
export function drawPlumberHouseCloseUp(ctx, t, cand, w, h, z = 3) {
  const floor = h - 48;
  const sky = ctx.createLinearGradient(0, 0, 0, floor);
  sky.addColorStop(0, '#78c8f0'); sky.addColorStop(1, '#a8e0f8');
  ctx.fillStyle = sky; ctx.fillRect(0, 0, w, h);
  ctx.fillStyle = HILL; ctx.fillRect(0, floor, w, h - floor);
  const cx = cand.id === 'bar' ? w * 0.43 : w / 2;
  drawPlumberHouseCandidate(ctx, t, { ...cand, S: cand.S * z }, cx, floor, true);
}

...
// the bare pair.  All three open on the line — that coin is what says the
// island has started — and none of them reaches the 5, which is the takeoff.
//
// THE ISLAND IS THE ROAD BETWEEN TWO HOLES TWO BEATS APART, and it is the one
// stretch on the cabinet that had nowhere to put a second coin.  Every other
// hole has a whole beat of clear road in front of it already; extra coins in
// front of every jump is the road's wallpaper again.
//
// AND THE BACK HALF IS THE HALF THAT COUNTS, because the lane was MEASURED: the
// hero comes down at 1.41 to 1.57 beats past the takeoff he is clearing, so the
// coin on the 1 is taken in the AIR and marks where the island begins, and the
// only coins his feet are under him for are the ones on 3 and 4.  That is also
// why the figures are sixteenths and not eighths — a coin on the 3 sits right
// at the touchdown, and there is nothing before it a hero can reach.
//
// AND `on34` DROPS THE 1 ALTOGETHER, which two of the six islands do.  It is
// the one figure here that pays everything it lays: measured with the bot over
// all three stages and all eight heroes, the coin on the 1 is collected 75% of
// the time and the one on the 2 78%, because at those two the hero is near the
// APEX of the jump out of the previous hole and sixty pixels over a pickup that
// sits at ten.  The 3 lands 99% and the 4 every time.  So an island that opens
// on the 3 has no marker and two coins you actually get, and one that opens on
// the 1 has a marker and a coin that is half a gesture.  Both readings are
// wanted; which island plays which is the variation.
//
// AND THE 2 IS KEPT ON PURPOSE, knowing what it does.  Slot 13 on stage 2 is
// the cabinet's only `on123`, and the coin on its 2 is collected THIRTY per
// cent of the time — the hero is forty pixels up as he passes it, coming down
// out of the hole on 12 — so on most passes that island sounds like `on13`.
// It is not a mistake and it is not to be tidied away: the figure still reads
// differently on the road, and an island that looks like three coins and pays
// like two is a fair thing for one of six to be.  If it ever wants fixing the
// fix is `on134`, which moves that third coin to where it always lands.
...
export const beatCharts = {
  ...
  2: {
    ...
    events: [
      ...
      // AND THE COIN BEFORE A HOLE IS A COUNT-IN.  A fill laid FORWARD from
      // this slot puts its second coin half a beat nearer the lip, which is
      // inside the clearance — so this was one coin for as long as the fills
      // only ran one way.  Laid backwards it ENDS where that coin stood and
      // every other coin in it runs away from the hazard: four sixteenths
      // closing on the beat you take off from.  It clears the card box for the
      // same reason, the whole run being behind its own line.
      { slot: 4, action: 'coin', ...sixteenthIn }, { slot: 5, action: 'pit' },
      // Six is between two holes and stays one coin: a pair of holes two beats
      // apart leaves a single beat line that a coin may stand on, and there is
      // nowhere on that island for a second one to go.
      { slot: 6, action: 'coin', ...on34 }, { slot: 7, action: 'pit' },
      ...
      // place on this cabinet where a pickup and a press are asked for in the
      // same stride, and it reads as "go and get this" over the top of "jump".
      { slot: 10, action: 'jump', type: 'beatBar' }, { slot: 11, action: 'coin' },
      { slot: 12, action: 'pit' }, { slot: 13, action: 'coin', ...on123 },
  ...
  3: {
    ...
    events: [
      ...
      { slot: 7, action: 'ability', type: 'cardBox', every: 3 },
      { slot: 8, action: 'coin', ...sixteenthIn }, { slot: 9, action: 'pit' },
      { slot: 10, action: 'coin', ...on13 }, { slot: 11, action: 'pit' },
      { slot: 12, action: 'coin', ...on34 }, { slot: 13, action: 'pit' },
      { slot: 14, action: 'coin', ...on134 }, { slot: 15, action: 'pit' },
    ],
  },
};

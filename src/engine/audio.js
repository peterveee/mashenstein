...
class AudioSys {
  ...
  // `hinged` is the Arcade Corner / back room variant. Same gesture — a leaf
  // moving air past you — but a hinged door is not a powered one: it swings in
  // a shorter arc, it pushes a duller pocket of air because it pivots rather
  // than travels, and it ends on timber rather than on a pneumatic seat. So the
  // band sits lower and narrower, the whole thing is quicker, and the accents
  // either end are wooden. Everything else is deliberately shared: these are
  // doors in the same wall and should sound like the same building.
  doorWhoosh(rising, hinged = false) {
    // A context that cannot make a buffer source is a test stub, not a browser.
    // Same guard explosion() carries for its own buffer, and for the same
    // reason: the cue is fired by rooms that tests drive headlessly.
    if (!this.ctx || typeof this.ctx.createBufferSource !== 'function') return;
    const t = this.cueAt();
    const q = this.cueGain * (hinged ? 0.9 : 1);
    const dur = hinged ? (rising ? 0.26 : 0.24) : (rising ? 0.34 : 0.32);
    const src = this.ctx.createBufferSource();
    src.buffer = this.noiseBuf; src.loop = true;
    const bp = this.ctx.createBiquadFilter();
    // Lower and tighter when hinged: a pivoting leaf never gets the air moving
    // the way a powered one does, and the high end of the sliding sweep is
    // exactly what makes that one sound driven.
    bp.type = 'bandpass'; bp.Q.value = hinged ? 1.9 : 1.1;
    const f0 = hinged ? (rising ? 300 : 880) : (rising ? 420 : 1500);
    const f1 = hinged ? (rising ? 880 : 260) : (rising ? 1500 : 380);
    ...
    this.noise(dur * 0.9, 0.06 * (hinged ? 1.15 : 1), 'lowpass', rising ? 520 : 430, 0, 0.5);
    // The one asymmetric detail, and the only thing that tells open from shut
    // with your eyes closed. The powered pair: a valve letting go, then a seat.
    // The hinged pair: a hinge taking the weight, then timber meeting timber —
    // drier and lower, because nothing here is pressurised.
    if (hinged) {
      if (rising) this.noise(0.03, 0.05, 'bandpass', 1900);
      else {
        this.noise(0.05, 0.10, 'lowpass', 300, dur - 0.045);
        this.osc('triangle', 150, 84, 0.08, 0.08, dur - 0.045);
      }
      return;
    }

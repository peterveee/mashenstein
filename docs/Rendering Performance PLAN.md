...
- **Layout mirroring surface — grew recently.** `resize()` (`renderer.js:574`) now has three modes (letterbox, `coverFit` visualiser fullscreen, `portraitFill` jukebox/dev), plus safe-area insets and separate input scale fields (`renderer.js:660-692`). Mode toggles: `setVisualiserFullscreen`, `setJukeboxPortrait`, `setDevPortraitFill`, `setDensityPin`, every density-rung change. `freshCanvasAfterWebglFailure()` clones and replaces `#game`; the sibling canvas must survive that. Mirror everything inside `resize()` itself so every caller is covered.

...
| **new** Patchwork fields: tractor ploughs a field every 12 s with 3 gulls behind it; cloud shadows slide | background | plumber-1 | **Grows** out of the near hills over `PLUMBER_FIELDS_IN` = 0.7–0.8 of the stage: its height ramps from flat to full relief about its foot (`opts.grow`), the base never moves; then stays to the finish | `drawPlumberPatchwork`, `ploughAndGulls` |

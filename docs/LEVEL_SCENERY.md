...
| **new** Patchwork fields: tractor ploughs a field every 12 s with 3 gulls behind it; cloud shadows slide | background | plumber-1 | **Rises** 60 px up from behind the near hills over `PLUMBER_FIELDS_IN` = 0.7–0.8 of the stage (`PLUMBER_FIELDS_RISE`), then stays to the finish | `drawPlumberPatchwork`, `ploughAndGulls` |
...
| **new** Speed trap, the SMILE! speed camera: flash every 3 s, then a **mugshot of the current hero** with GOTCHA! and a $1987 fine; the patrol car's light bar flashes behind | background | speed-2 | Once, pinned to a distance: `DESERT_TRAP_AT_PX` = 320 world px, so mid-picture about 3 s in (before the copter arrives) and gone by ≈10% | `drawDesertSpeedTrap(…, heroId)` |
| **new** Jet: fighter goes supersonic with a vapour cone, shock ring and lingering contrail | background (flypast) | speed-3 | Once, over `camX` from 0.45 of the stage for `DESERT_JET_PASS` = 1600 world px (≈45–59%) | `drawDesertJet` |

...
> 2. **The tape rolls all run on every device.** Recording used to start when a
>    capsule was collected, which made the rewind's length depend on how long ago
>    that was — a hit one second after the grab rewound one second. See
>    **The tape is always rolling**.
> 3. **It has no button. It fires itself.** The design had the player press RWD/a key to
...
| snapshot ring | 45 records (3s), always rolling | 150 records (10s), always |
| audio capture node | up for the run, torn down on the way out | boot-time, permanent |
| how it is spent | automatically, on your next mistake | automatically, on your next mistake |

`Input.rewindAvailable()` still gates the ring SIZE and the capture node's lifetime. It
no longer gates *access* — nothing about who is handed a capsule depends on the device —
and it no longer gates recording at all.
...
The first build tied recording to the charge: on touch the ring rolled from collection
until the capsule fired or the level ended, and nothing before or after. That is the
cheapest possible reading of "record for the armed window", and it is wrong for one
reason — **a rewind rewinds the past**. A tape that starts when the capsule is collected
holds only the seconds SINCE the collection, so the rewind's reach is however long ago
that was. Measured on the touch path: a hit one second after the grab rewound 0.95s and
156 world px, against the 2.97s and 500px the same capsule gives when the tape is full.
A save whose size depends on something invisible is not the three seconds the capsule
promises.

So the ring records for the whole run everywhere, and the capsule always finds its three
seconds waiting. Cost on touch is the 45-record ring (still a third of desktop's 150,
still pooled, still allocated lazily and once) plus the capture pass at 15Hz for the rest
of the run: **0.009ms per frame measured** over a minute of headless play, 0.034ms
against 0.025ms with `?norewind`. The audio recorder follows the same rule — up for any
run that can rewind, torn down on the way out — so the reversed audio covers the same
tape the picture does.

`Input.rewindAvailable()` therefore decides only the ring's SIZE and whether the capture
node outlives the run. There is no armed/disarmed transition left to handle, and
`updateRewindArm` is gone with it.
...
- `grab('rewind')` stores `{ level, persistent: true }`; `Powerups.update` does not tick
  persistent entries. The fixed rewind length still lives in `run.js`, and so does the
  tape: collecting the capsule changes nothing about recording.
...
- Ring sizing in the constructor (`run.js:743`):
  ...
  construction.
- `recordRewindFrame` gate is `if (REWIND_DISABLED || this.beatLock) return;` — no
  charge test. (It was `!Input.rewindAvailable() && !this.powerups.active.rewind`; see
  **The tape is always rolling** for why that came out.)
- ~~**One armed/disarmed transition hook**~~ **REMOVED** — it was, immediately after `this.powerups.update(dt)`,

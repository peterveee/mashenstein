...
- Completed: a hidden `TITLE 3X PROFILE` run now pins WebGL at 3× and
  measures full title, no-sky, no-bloom, no-parade and world-only stages. It
  reports presented FPS, Canvas2D paint time, renderer submission time, and
...
The hidden `TITLE 3X PROFILE` diagnostic run pins WebGL at 3×. It does not draw
a changing report through the WebGL overlay while measuring; it stores the
results and shows them only after the run.
